...
class FinalRealisticMAPE:
    ...
    def create_directional_target(self, data: pd.DataFrame, threshold: float = 0.01) -> pd.Series:
        """方向性ターゲット（上昇/下降/中立）"""
        close = data['Close']
    ...
    def train_directional_classifier(self, X: pd.DataFrame, y: pd.Series) -> Tuple[object, float]:
        """方向性分類器の訓練"""
        models = {
    ...
    def convert_direction_to_mape(self, actual_returns: pd.Series,
        ...
        for i, (direction, probs) in enumerate(zip(predicted_directions, direction_probs)):
            if direction == 2:  # 上昇予測
    ...
    def test_realistic_system(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:10]:
            try:
                ...
                # 分類器訓練
                classifier, accuracy = self.train_directional_classifier(X_train, y_train)
        ...
        # 結果分析
        if all_results:
            valid_results = [r for r in all_results if np.isfinite(r['mape']) and r['mape'] < 200]
