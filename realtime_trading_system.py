...
class RealTimeDataProvider:
    ...
    def get_realtime_data(self, symbol: str, period: str = "1d") -> Optional[pd.DataFrame]:
        """リアルタイムデータ取得"""
        try:
            # 日本株式の場合は.Tを追加
...
class Pattern846Detector:
    ...
    def detect_846_pattern(self, data: pd.DataFrame) -> Dict:
        """84.6%成功パターンの検出"""
        if len(data) < 50:
            return {"signal": 0, "confidence": 0, "reason": "データ不足"}
        ...
        # 最新10日間の確認
        if len(data) >= 10:
            recent_up = strong_uptrend.tail(10).sum()
            recent_down = strong_downtrend.tail(10).sum()

            if recent_up >= 7:
                signal = 1  # 買いシグナル
                confidence = 0.846 + (recent_up - 7) * 0.02  # 継続日数で信頼度調整
                reason = f"強い上昇トレンド継続: {recent_up}日"

            elif recent_down >= 7:
                signal = -1  # 売りシグナル
                confidence = 0.846 + (recent_down - 7) * 0.02
                reason = f"強い下降トレンド継続: {recent_down}日"
        ...
        if signal != 0:
            ...
            if recent_volume < vol_avg.iloc[-1] * 0.5:
                confidence *= 0.8  # ボリューム不足で信頼度低下
                reason += " (ボリューム不足)"

        return {
            "signal": signal,
            "confidence": min(confidence, 0.99),  # 最大99%
            "reason": reason,
            "sma_10": sma_10.iloc[-1] if len(sma_10) > 0 else 0,
            "sma_20": sma_20.iloc[-1] if len(sma_20) > 0 else 0,
...
class RiskManager:
    ...
    def calculate_position_size(self, symbol: str, signal: int, confidence: float,
        ...
        max_exposure = self.current_capital * self.settings.realtime.max_total_exposure_pct
        ...
        position_ratio = min(position_ratio, self.settings.realtime.max_position_size_pct)
    ...
    def update_positions(self, symbol: str, action: str, size: int, price: float):
        """ポジション更新"""
        if action == "BUY":
            if symbol in self.positions:
                ...
                self.positions[symbol] = {
                    ...
                    'stop_loss': price * (1 - self.settings.realtime.default_stop_loss_pct),
                    'take_profit': price * (1 + self.settings.realtime.default_take_profit_pct)
                }
            ...
            if symbol in self.positions:
                if self.positions[symbol]['size'] >= size:
...
class OrderExecutor:
    ...
    def execute_order(self, symbol: str, action: str, size: int, price: float,
        ...
        if self.mode == "simulation":
            # シミュレーションモード
            logging.info(f"シミュレーション注文: {action} {symbol} {size}株 @{price:.0f}円 (信頼度: {confidence:.1%})")
...
class RealTimeTradingSystem:
    ...
    def _monitoring_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # 市場時間確認
                if self.settings.realtime.market_hours_only and not self._is_market_hours():
                    time.sleep(300)  # 市場外は5分間隔
                    continue

                # 全銘柄をスキャン
                for symbol in list(self.settings.target_stocks.keys())[:5]:  # 最初の5銘柄でテスト
                    try:
    ...
    def _process_symbol(self, symbol: str):
        ...
        if pattern_result['signal'] != 0 and pattern_result['confidence'] >= self.settings.realtime.pattern_confidence_threshold:
            ...
            if position_info['size'] > 0:
                ...
                if order_result['status'] == 'executed':
                    ...
                    logging.info(f"注文執行完了: {symbol} {action} {position_info['size']}株")
    ...
    def get_status_report(self) -> Dict:
        ...
        total_return = (total_value - self.risk_manager.initial_capital) / self.risk_manager.initial_capital * 100
...
def main():
    ...
    try:
        ...
        print(f"総収益: {final_status['total_value'] - final_status['initial_capital']:,.0f}円")
