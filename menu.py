...
def show_main_menu():
    ...
    print("  2. [グラフ] デモ取引シミュレーション")
    print("  3. [最適] TSE4000から最適30銘柄選出")
    print()
    print(f"{Colors.YELLOW}■ ダッシュボード{Colors.ENDC}")
    print("  4. [PC] Webダッシュボードを起動")
    print("  5. [電話] 投資アドバイザーCUI")
    print()
    print(f"{Colors.YELLOW}■ システム管理{Colors.ENDC}")
    print("  6. [ロケット] プロセス管理画面")
    print("  7. [チャート] パフォーマンス監視")
    print("  8. [メモ] ログ分析レポート")
    print()
    print(f"{Colors.YELLOW}■ データ・テスト{Colors.ENDC}")
    print("  9. [受信] 最新株価データ取得")
    print(" 10. [実験] テスト実行（カバレッジ93%）")
    print()
...
def run_87_prediction():
    ...
    try:
        ...
        if result['precision_87_achieved']:
            print(f"87%達成: {Colors.GREEN}[成功] YES{Colors.ENDC}")
        else:
            print(f"87%達成: {Colors.RED}[失敗] NO{Colors.ENDC}")
...
def run_optimal_selection():
    """最適30銘柄選出"""
    clear_screen()
    print(f"{Colors.CYAN}【TSE4000最適30銘柄選出】{Colors.ENDC}\n")

    print("東証4000銘柄から最適な30銘柄を選出します。")
    print("12セクターから分散投資に適した銘柄を自動選択。")
    print("過去実績: +17.32%の利益達成\n")

    print("選択オプション:")
    print("  1. 最適30銘柄で予測実行")
    print("  2. TSE4000最適化分析")
    print("  3. 既存の最適銘柄リスト表示")
    print("  0. 戻る")
    ...
    try:
        if choice == "1":
            print(f"\n{Colors.YELLOW}最適30銘柄で予測実行中...{Colors.ENDC}")
            os.system("python optimal_30_prediction_system.py")
        elif choice == "2":
            ...
            print(f"\n{Colors.GREEN}現在の最適30銘柄:{Colors.ENDC}")
            # 設定から最適銘柄を表示
            optimal_stocks = [
                "7203 (トヨタ自動車)", "6758 (ソニーグループ)",
                "9432 (NTT)", "8306 (三菱UFJ)", "6861 (キーエンス)",
                "4519 (中外製薬)", "6098 (リクルートHD)", "9984 (ソフトバンクG)"
            ]
            for i, stock in enumerate(optimal_stocks[:8], 1):
                print(f"  {i:2d}. {stock}")
            print("  ... 他22銘柄")
    except Exception as e:
        print(f"{Colors.RED}実行エラー: {e}{Colors.ENDC}")
