...
class Direct846Enhancement:
    ...
    def _analyze_direct_enhancement(self, all_results: List[Dict], enhanced_results: List[Dict]) -> Dict:
        """84.6%直接強化結果の分析"""
        if not all_results:
        ...
        if enhanced_results:
            ...
            print("\n84.6%突破達成銘柄:")
            for r in sorted(enhanced_results, key=lambda x: x['accuracy'], reverse=True):
