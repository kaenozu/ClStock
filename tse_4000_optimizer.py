...
class TSE4000Optimizer:
    ...
    def _initialize_tse_universe(self) -> Dict[str, List[str]]:
        ...
        return {
            # 金融・銀行（10銘柄）
            'finance': ['8306.T', '8411.T', '8316.T', '8031.T', '8002.T', 
                       '8001.T', '8058.T', '8750.T', '8725.T', '8771.T'],

            # テクノロジー・IT（15銘柄）
            'tech': ['6758.T', '9984.T', '4689.T', '9433.T', '9432.T', 
                    '6861.T', '6367.T', '6701.T', '8035.T', '4519.T',
                    '3765.T', '4307.T', '4751.T', '2432.T', '4385.T'],

            # 自動車・輸送（12銘柄）
            'automotive': ['7203.T', '7267.T', '7269.T', '6902.T', '7201.T', 
                          '7261.T', '9020.T', '9021.T', '9022.T',
                          '5401.T', '5411.T', '7011.T'],

            # 製造業・重工業（12銘柄）
            'manufacturing': ['6501.T', '6503.T', '6502.T', '6504.T', '7751.T', 
                             '6770.T', '6752.T', '6954.T', '6724.T', '6703.T', 
                             '7012.T', '5201.T'],

            # 消費・小売（10銘柄）
            'consumer': ['9983.T', '3382.T', '8267.T', '3099.T', '2914.T', 
                        '2802.T', '2801.T', '4523.T', '4578.T', '4902.T'],

            # エネルギー・資源（8銘柄）
            'energy': ['1605.T', '1332.T', '5020.T', '3865.T', '1801.T', 
                      '1802.T', '1803.T', '5101.T'],

            # ヘルスケア・製薬（8銘柄）
            'healthcare': ['4502.T', '4503.T', '4519.T', '4901.T', '4911.T', 
                          '4922.T', '4568.T', '4021.T'],

            # 不動産・建設（8銘柄）
            'realestate': ['8802.T', '1925.T', '1963.T', '1801.T', '1808.T', 
                          '1812.T', '1893.T', '1928.T'],

            # 通信・メディア（6銘柄）
            'telecom': ['9432.T', '9433.T', '9434.T', '4751.T', '2432.T', '4324.T'],

            # 化学・材料（8銘柄）
            'chemicals': ['4063.T', '4183.T', '4208.T', '4452.T', '3407.T', 
                         '4188.T', '4004.T', '4005.T'],
    ...
    def _create_stock_profile(self, symbol: str, stock_data: pd.DataFrame) -> StockProfile:
        """株価プロファイルの作成"""
        ...
        combined_score = self._calculate_combined_score(profit_potential, diversity_score, volatility)
    ...
    def _calculate_trend_strength(self, ma_short: pd.Series, ma_long: pd.Series) -> float:
        """トレンド強度計算"""
        if ma_long.iloc[-1] > 0:
    ...
    def _calculate_combined_score(self, profit_potential: float, diversity_score: float, volatility: float) -> float:
        """総合スコア計算"""
    ...
    def calculate_diversity_score(self, symbol: str, volatility: float, market_cap: float) -> float:
        """多様性スコア計算"""
    ...
    def _get_sector_weight(self, symbol: str) -> float:
        ...
        sector = self.determine_sector(symbol)
        sector_weights = {
            'finance': 1.0, 'tech': 1.2, 'automotive': 1.0, 'manufacturing': 0.9,
            'consumer': 1.1, 'energy': 1.3, 'healthcare': 1.2, 'realestate': 0.8,
            'telecom': 1.0, 'chemicals': 0.9, 'food': 0.7, 'transport': 1.1
        }
    ...
    def optimize_portfolio(self, profiles: List[StockProfile], target_size: int = DEFAULT_TARGET_SIZE) -> List[StockProfile]:
    ...
    def _get_sector_best_stocks(self, profiles: List[StockProfile]) -> Dict[str, StockProfile]:
        """セクター別ベスト銘柄の取得"""
        sector_best = {}
        for profile in profiles:
            if (profile.sector not in sector_best or 
                profile.combined_score > sector_best[profile.sector].combined_score):
    ...
    def _execute_trading_strategy(self, symbol: str, stock_data: pd.DataFrame, portfolio_state: Dict):
        """トレーディング戦略の実行"""
        close = stock_data['Close']
        ...
        for i in range(LONG_MA_PERIOD, len(close)-1):
            ...
            # 買いシグナル処理
            if self._is_buy_signal(ma_short, ma_long, i, symbol, portfolio_state['positions']):
                self._execute_buy_order(symbol, current_price, position_size, stock_data.index[i], portfolio_state)

            # 売りシグナル処理
            elif self._is_sell_signal(ma_short, ma_long, i, symbol, portfolio_state['positions']):
                self._execute_sell_order(symbol, current_price, stock_data.index[i], portfolio_state)

    def _is_buy_signal(self, ma_short: pd.Series, ma_long: pd.Series, i: int, symbol: str, positions: Dict) -> bool:
        """買いシグナルの判定"""
        return (ma_short.iloc[i] > ma_long.iloc[i] and
                ma_short.iloc[i-1] <= ma_long.iloc[i-1] and
                symbol not in positions)

    def _is_sell_signal(self, ma_short: pd.Series, ma_long: pd.Series, i: int, symbol: str, positions: Dict) -> bool:
        """売りシグナルの判定"""
        return (ma_short.iloc[i] < ma_long.iloc[i] and
                ma_short.iloc[i-1] >= ma_long.iloc[i-1] and
                symbol in positions)

    def _execute_buy_order(self, symbol: str, price: float, position_size: float, date, portfolio_state: Dict):
        """買い注文の実行"""
    ...
    def display_optimization_results(self, results: Dict, all_profiles: List[StockProfile]):
        """最適化結果の表示"""
        print("\n" + "=" * 80)
        print("最適化結果サマリー")
        print("=" * 80)
    ...
    def _display_portfolio_comparison(self, results: Dict):
        ...
        for size, result in results.items():
            ...
            print(f"{size:>6} {backtest['return_rate']:>+7.2f}% {backtest['total_return']:>+11,.0f}円 "
                  f"{backtest['total_trades']:>7} {performance_score:>+7.1f}")

    def _display_optimal_portfolio_details(self, best_result: Tuple[int, Dict], results: Dict):
        """最適ポートフォリオ詳細の表示"""
    ...
    def _display_sector_breakdown(self, selected_profiles: List[StockProfile]):
        ...
        for sector, profiles in sector_groups.items():
            print(f"\n【{sector.upper()}】")
            for profile in profiles:
                print(f"  {profile.symbol}: 総合スコア {profile.combined_score:.1f} "
    ...
    def _display_top_performers(self, all_profiles: List[StockProfile]):
        ...
        print(f"\n全体トップ10銘柄:")
        print("-" * 50)
        top_performers = sorted(all_profiles, key=lambda x: x.combined_score, reverse=True)[:10]
        for i, profile in enumerate(top_performers, 1):
            print(f"{i:2d}. {profile.symbol} [{profile.sector}] スコア: {profile.combined_score:.1f}")

    def _finalize_results_display(self, results: Dict, all_profiles: List[StockProfile]):
        """結果表示の最終処理"""
    ...
    def _print_save_success(self, save_result: Dict):
        ...
        print(f"最適解: {save_result['optimal_portfolio_size']}銘柄で{save_result['expected_return']}期待利益")

    def save_optimization_results(self, results: Dict, all_profiles: List[StockProfile]):
        """最適化結果をファイルに保存"""
        ...
        return self._save_all_result_files(timestamp, best_size, selected_profiles, 
                                         backtest_result, results, all_profiles)

    def _save_all_result_files(self, timestamp: str, best_size: int, selected_profiles: List[StockProfile],
                              backtest_result: Dict, results: Dict, all_profiles: List[StockProfile]) -> Dict:
        """全結果ファイルの保存"""
        csv_file = self._save_csv_file(timestamp, selected_profiles)
        json_file = self._save_json_file(timestamp, best_size, backtest_result, results, 
                                       selected_profiles, all_profiles)
        report_file = self._save_report_file(timestamp, best_size, backtest_result, 
                                           selected_profiles, results)
    ...
    def _save_csv_file(self, timestamp: str, selected_profiles: List[StockProfile]) -> str:
        """CSVファイルの保存"""
        csv_filename = f"optimal_portfolio_{timestamp}.csv"
        with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([
                'Symbol', 'Sector', 'Combined_Score', 'Profit_Potential', 
                'Diversity_Score', 'Volatility', 'Market_Cap'
            ])
            
            for profile in selected_profiles:
                writer.writerow([
                    profile.symbol, profile.sector, f"{profile.combined_score:.2f}",
                    f"{profile.profit_potential:.2f}", f"{profile.diversity_score:.2f}",
                    f"{profile.volatility:.4f}", f"{profile.market_cap:.0f}"
                ])
        
        print(f"[保存完了] 最適ポートフォリオCSV: {csv_filename}")
        return csv_filename

    def _save_json_file(self, timestamp: str, best_size: int, backtest_result: Dict,
                       results: Dict, selected_profiles: List[StockProfile], 
                       all_profiles: List[StockProfile]) -> str:
        """JSONファイルの保存"""
        json_data = self._create_json_data(timestamp, best_size, backtest_result, 
                                         results, selected_profiles, all_profiles)
    ...
    def _create_json_data(self, timestamp: str, best_size: int, backtest_result: Dict,
                         results: Dict, selected_profiles: List[StockProfile], 
                         all_profiles: List[StockProfile]) -> Dict:
        """JSON用データの作成"""
        return {
    ...
    def _create_optimal_portfolio_data(self, selected_profiles: List[StockProfile]) -> List[Dict]:
        """最適ポートフォリオデータの作成"""
        portfolio_data = []
        for profile in selected_profiles:
    ...
    def _create_sector_analysis_data(self, selected_profiles: List[StockProfile]) -> Dict:
        """セクター分析データの作成"""
        sector_groups = {}
        for profile in selected_profiles:
            if profile.sector not in sector_groups:
        ...
        for sector, profiles in sector_groups.items():
            sector_analysis[sector] = {
                "count": len(profiles),
                "avg_score": round(sum(p.combined_score for p in profiles) / len(profiles), 2),
                "avg_profit_potential": round(sum(p.profit_potential for p in profiles) / len(profiles), 2),
                "symbols": [p.symbol for p in profiles]
            }
        return sector_analysis

    def _create_top_performers_data(self, all_profiles: List[StockProfile]) -> List[Dict]:
        """トップパフォーマーデータの作成"""
        top_performers = sorted(all_profiles, key=lambda x: x.combined_score, reverse=True)[:10]
        performers_data = []
        for i, profile in enumerate(top_performers, 1):
    ...
    def _save_report_file(self, timestamp: str, best_size: int, backtest_result: Dict,
                         selected_profiles: List[StockProfile], results: Dict) -> str:
        """レポートファイルの保存"""
        report_filename = f"investment_recommendation_{timestamp}.txt"
        with open(report_filename, 'w', encoding='utf-8') as reportfile:
            self._write_report_content(reportfile, timestamp, best_size, backtest_result, 
                                     selected_profiles, results)
        
        print(f"✅ 投資推奨レポート保存: {report_filename}")
        return report_filename

    def _write_report_content(self, reportfile, timestamp: str, best_size: int, 
                             backtest_result: Dict, selected_profiles: List[StockProfile], 
                             results: Dict):
        """レポート内容の書き込み"""
        reportfile.write("=" * 80 + "\n")
        reportfile.write("TSE 4000銘柄最適化投資推奨レポート\n")
        reportfile.write("=" * 80 + "\n")
        reportfile.write(f"作成日時: {datetime.now().strftime('%Y年%m月%d日 %H:%M:%S')}\n\n")
    ...
    def _write_strategy_summary(self, reportfile, best_size: int, backtest_result: Dict):
        """投資戦略サマリーの書き込み"""
    ...
    def _write_sector_allocation(self, reportfile, selected_profiles: List[StockProfile], 
                                best_size: int, results: Dict):
        """セクター別投資配分の書き込み"""
        ...
        for sector, info in sector_analysis.items():
            ...
            reportfile.write(f"{sector.upper()}: {info['count']}銘柄 ({percentage:.1f}%) ")
            reportfile.write(f"- 平均スコア: {info['avg_score']:.1f}\n")
            reportfile.write(f"  推奨銘柄: {', '.join(info['symbols'])}\n\n")

    def _write_top_recommendations(self, reportfile, selected_profiles: List[StockProfile]):
        """トップ推奨銘柄の書き込み"""
        reportfile.write("💎 トップ推奨銘柄（上位5銘柄）\n")
        reportfile.write("-" * 40 + "\n")
        top_5 = sorted(selected_profiles, key=lambda x: x.combined_score, reverse=True)[:5]
