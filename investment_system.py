        ...
        for symbol in symbols:  # 全指定銘柄でテスト
            try:
                ...
                for signal_date in signal_dates[-5:]:  # 最新5回のシグナル
                    ...
                    # ボラティリティ計算
                    volatility = data['Close'].pct_change().rolling(20).std().loc[signal_date]

                    if signal_value == 1:  # 買いシグナル
                        position_value = self.calculate_position_size(symbol, confidence, volatility)
                        ...
                        if quantity > 0:
                            ...
                            if success:
                                total_signals += 1
                                print(f"    {signal_date.date()}: 買い {quantity}株 @{current_price:.0f}円")
                                ...
                                if future_date in data.index:
                                    ...
                                    sell_success = self.execute_trade(
                                        symbol, 'SELL', future_price, quantity,
                                        future_date, confidence
                                    )
                                    if sell_success:
                                        profit = (future_price - current_price) * quantity
                                        if profit > 0:
                                            successful_trades += 1
                                        print(f"    {future_date.date()}: 売り {quantity}株 @{future_price:.0f}円 (損益: {profit:,.0f}円)")
    ...
    def _analyze_backtest_results(self, total_signals, successful_trades):
        ...
        if len(self.transaction_history) > 0:
            ...
            for trade in self.transaction_history[-5:]:
                print(f"  {trade['date'].date()}: {trade['action']} {trade['symbol']} "
