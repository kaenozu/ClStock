...
class IntegratedDeepPredictor:
    ...
    def train(
        ...
        try:
            ...
            self.training_history = (
                history.history if hasattr(history, "history") else {}
            )
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        try:
            if not self.is_trained:
                raise ValueError(
                    "モデルが訓練されていません。train()を呼び出してください。",
                )
    ...
    def explain_prediction(
        ...
        try:
            if self.interpretability is None:
                raise ValueError(
                    "解釈性分析がセットアップされていません。setup_interpretability()を呼び出してください。",
                )
    ...
    def get_trading_recommendation(self, symbol: str, period: str = "6m") -> float:
        ...
        if self.reinforcement_trader.model is None:
            raise ValueError(
                "取引戦略が最適化されていません。optimize_trading_strategy()を呼び出してください。",
            )
