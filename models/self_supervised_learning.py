...
from tensorflow.keras.layers import Input, Dense, Conv1D, MaxPooling1D, Flatten, LSTM
...
class SelfSupervisedModel:
    ...
    def _build_model(self):
        ...
        decoded = Dense(np.prod(self.input_shape), activation='linear')(decoded)  # 出力層のサイズ調整
        
        # Autoencoderモデル
        self.autoencoder = Model(input_layer, decoded)
        
        # エンコーダーモデル
        self.encoder = Model(input_layer, self.autoencoder.get_layer('encoded_layer').output)
...
class TemporalSelfSupervisedModel:
    ...
    def _build_model(self):
        ...
        encoded = LSTM(self.encoding_dim, return_sequences=False, name='encoded_layer')(encoded)
        ...
        logger.info(f"時系列自己教師あり学習モデル構築完了: {self.autoencoder.summary()}")
