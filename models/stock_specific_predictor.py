...
class StockSpecificPredictor:
    ...
    def analyze_symbol_characteristics(self, symbol: str, data: pd.DataFrame) -> Dict[str, Any]:
        """銘柄固有の特性を分析"""
        try:
            if len(data) < 100:
    ...
    def _calculate_trend_persistence(self, close: pd.Series) -> float:
        ...
        avg_streak = np.mean(up_streaks + down_streaks) if (up_streaks + down_streaks) else 1
    ...
    def create_symbol_features(self, symbol: str, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 特徴量リストを保存
        feature_columns = [col for col in df.columns if col not in ['Open', 'High', 'Low', 'Close', 'Volume', 'Symbol', 'CompanyName']]
    ...
    def _add_846_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 84.6%の核心パターン
        df['ma_bullish'] = ((df['sma_10'] > df['sma_20']) &
                           (df['sma_20'] > df['sma_50']) &
                           (close > df['sma_10'])).astype(int)

        df['ma_bearish'] = ((df['sma_10'] < df['sma_20']) &
                           (df['sma_20'] < df['sma_50']) &
                           (close < df['sma_10'])).astype(int)
    ...
    def _add_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ボラティリティブレイクアウト
        df['vol_breakout'] = (df['volatility_ratio'] > df['volatility_ratio'].rolling(20).quantile(0.8)).astype(int)
    ...
    def _add_liquidity_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 価格・出来高関係
        df['price_volume_trend'] = (close.pct_change() * df['volume_ratio']).rolling(5).mean()
    ...
    def train_symbol_model(self, symbol: str, lookback_period: str = "2y") -> Dict[str, Any]:
        """個別銘柄モデルを訓練"""
        try:
            ...
            df_features['future_return'] = df_features['Close'].pct_change(prediction_days).shift(-prediction_days)
            df_features['target'] = (df_features['future_return'] > target_return).astype(int)
            ...
            model_info = {
                ...
                'symbol_characteristics': self.analyze_symbol_characteristics(symbol, data)
            }
            ...
            logger.info(f"✅ {symbol} モデル訓練完了: {best_model_name} {best_score:.3f}")
            return model_info
    ...
    def predict_symbol(self, symbol: str, current_data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        """個別銘柄予測実行"""
        try:
    ...
    def batch_train_all_symbols(self, symbols: Optional[List[str]] = None) -> Dict[str, Dict[str, Any]]:
        """全銘柄の一括訓練"""
        if symbols is None:
