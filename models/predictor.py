...
class StockPredictor:
    ...
    def calculate_score(self, symbol: str) -> float:
        ...
        try:
            ...
            # モメンタム分析（価格変化率）
            price_momentum_5d = (current_price - data["Close"].iloc[-6]) / data[
                "Close"
            ].iloc[-6]
            price_momentum_20d = (current_price - data["Close"].iloc[-21]) / data[
                "Close"
            ].iloc[-21]
    ...
    def predict_return_rate(self, symbol: str) -> float:
        ...
        try:
            ...
            # 5. モメンタム分析（複数時間軸）
            momentum_3d = (current_price - data["Close"].iloc[-4]) / data["Close"].iloc[
                -4
            ]
            momentum_7d = (current_price - data["Close"].iloc[-8]) / data["Close"].iloc[
                -8
            ]
            ...
            if abs(predicted_return) > base_range:
                if predicted_return > 0:
                    predicted_return = (
                        base_range + (predicted_return - base_range) * 0.3
                    )
                else:
                    predicted_return = (
                        -base_range + (predicted_return + base_range) * 0.3
    ...
    def _identify_strong_trend_period(
        ...
        # より緩い条件での再試行
        simple_trend = (
            (close > sma_20) & (sma_20.pct_change(5) > 0.002)
        ) | (  # シンプル上昇
            (close < sma_20) & (sma_20.pct_change(5) < -0.002)
        )  # シンプル下降
    ...
    def _create_trend_direction_features(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        # 移動平均の傾き
        features["sma10_slope"] = (
            float(sma_10.pct_change(5).iloc[-1]) if len(sma_10) >= 6 else 0.0
        )
        features["sma20_slope"] = (
            float(sma_20.pct_change(5).iloc[-1]) if len(sma_20) >= 6 else 0.0
        )
        ...
        features["price_momentum_5d"] = (
            float(close.pct_change(5).iloc[-1]) if len(close) >= 6 else 0.0
        )
        features["price_momentum_10d"] = (
            float(close.pct_change(10).iloc[-1]) if len(close) >= 11 else 0.0
        )

        # 連続上昇/下降日数
        daily_change = close.pct_change() > 0
        features["consecutive_up"] = (
            float(daily_change.rolling(5).sum().iloc[-1])
            if len(daily_change) >= 5
            else 0.0
        )
        ...
        features["volume_support"] = (
            float(volume.iloc[-1] > vol_avg.iloc[-1]) if len(vol_avg) >= 1 else 0.0
        )
    ...
    def enhanced_predict_with_direction(self, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            # 方向性予測で既存予測を調整
            if (
                direction_result["is_strong_trend"]
                and direction_result["confidence"] > 0.6
            ):
                # 高信頼度の方向性予測がある場合は調整
                ...
                if direction_factor == existing_direction:
                    ...
                    enhanced_return = existing_return * (
                        1 + direction_result["confidence"] * 0.5
                    )
                else:
            ...
            direction_weight = (
                ...
                if direction_result["is_strong_trend"]
                else 0
            )
            base_weight = 1 - direction_weight
            combined_accuracy = (
                direction_result["accuracy_expected"] * direction_weight
                + 0.6 * base_weight
            )  # 既存手法の推定精度60%
