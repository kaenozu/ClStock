...
class ReturnRateBacktester:
    ...
    def calculate_directional_accuracy(self, actual: np.ndarray, predicted: np.ndarray) -> float:
        ...
        correct_direction = (actual_direction[significant_moves] == predicted_direction[significant_moves]).mean()
        return correct_direction * 100

    def walk_forward_backtest(self, symbols: List[str], start_date: str = "2023-01-01",
        ...
        while current_date < end_dt - timedelta(days=self.prediction_days + 5):
            ...
            for symbol in symbols[:10]:  # 最初の10銘柄でテスト
                try:
                    ...
                    # 実際のリターン取得
                    actual_return = self._get_actual_return(symbol, current_date, self.prediction_days)
                    ...
                    logger.warning(f"Prediction failed for {symbol} at {current_date}: {str(e)}")
                    continue
    ...
    def _get_actual_return(self, symbol: str, start_date: datetime, days: int) -> float:
        ...
        try:
            ...
            # 開始日に最も近いデータを見つける
            start_price_idx = data.index.get_indexer([start_date_utc], method='nearest')[0]
