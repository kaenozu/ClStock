...
def demo_position_sizing():
    ...
    for condition in conditions:
        ...
        print(
            f"{condition['name']}: {result.shares}株, 価値: {result.position_value:,.0f}円"
        )
...
def demo_exit_strategy():
    ...
    result = exit_strategy.calculate_exit_strategy(
        entry_price=entry_price, df=df, direction="long", market_condition="normal"
    )
...
def demo_comprehensive_system():
    ...
    buy_result = portfolio.execute_trade_safely(
        symbol="7203", action="buy", price=3100, market_data=market_data  # 現在価格
    )
    ...
    if buy_result:
        ...
        print(
            f"提案損切り: {buy_result['stop_loss']:.0f}円, 利確: {buy_result['take_profit']:.0f}円"
        )
    else:
    ...
    print(
        f"リスクレベル: {risk_assessment.risk_level}, スコア: {risk_assessment.risk_score:.3f}"
    )
