...
class FinalBreakthrough846:
    ...
    def create_trend_target(self, data: pd.DataFrame, prediction_days: int = 3) -> pd.Series:
    ...
    def create_power_ensemble(self) -> VotingClassifier:
        ...
        models = [
            ...
            # 強力な追加モデル群
            ('rf_power', RandomForestClassifier(
                n_estimators=150,
                max_depth=7,
                min_samples_split=8,
                min_samples_leaf=3,
        ...
        for symbol in symbols[:35]:
            try:
                ...
                if strong_trend_mask.sum() < 30:
                    print(f"  スキップ: 強いトレンド期間不足 ({strong_trend_mask.sum()})")
                    continue
    ...
    def _analyze_final_breakthrough(self, all_results: List[Dict], breakthrough_results: List[Dict]) -> Dict:
        """最終突破結果の分析"""
        if not all_results:
        ...
        if breakthrough_results:
            ...
            for r in sorted(breakthrough_results, key=lambda x: x['accuracy'], reverse=True):
                hc_info = f" (高信頼度: {r['high_conf_accuracy']:.1%})" if r['high_conf_accuracy'] > 0 else ""
        ...
        for i, result in enumerate(top_results, 1):
            hc_info = f" (高信頼度: {result['high_conf_accuracy']:.1%})" if result['high_conf_accuracy'] > 0 else ""
            mark = "***" if result['accuracy'] > 0.846 else "○○○" if result['accuracy'] >= 0.84 else "○○" if result['accuracy'] >= 0.8 else "○" if result['accuracy'] >= 0.75 else ""
            print(f"  {i}. {result['symbol']}: {result['accuracy']:.1%}{hc_info} {mark}")
