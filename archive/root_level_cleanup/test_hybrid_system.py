...
def run_hybrid_system_tests() -> Dict[str, Any]:
    ...
    try:
        ...
        # パフォーマンステスト
        print("\n3. パフォーマンステスト")
        test_results['performance_tests'] = test_hybrid_performance(hybrid_system, test_symbols)

        # 統合機能テスト
        print("\n4. 統合機能テスト")
        test_results['integration_tests'] = test_integration_features(hybrid_system, test_symbols)
...
def test_prediction_modes(hybrid_system, test_symbols: List[str]) -> Dict[str, Any]:
    ...
    for mode in [PredictionMode.SPEED_PRIORITY, PredictionMode.ACCURACY_PRIORITY,
                 PredictionMode.BALANCED, PredictionMode.AUTO]:
        ...
        for symbol in test_symbols:
            try:
                ...
                print(f"     {symbol}: 予測値={result.prediction:.1f}, "
        ...
        print(f"     総時間: {mode_data['total_time']:.3f}秒, "
    ...
    for symbol in test_symbols:
        try:
            ...
            for i in range(3):
                ...
                print(f"     {symbol} 回{i+1}: モード={result.metadata.get('mode_used', 'unknown')}, "
                      f"システム={result.metadata.get('system_used', 'unknown')}")
...
def display_test_results(results: Dict[str, Any]):
    ...
    if 'mode_tests' in results:
        ...
        for mode, data in mode_tests.items():
            if data['predictions']:
                ...
                print(f"    銘柄あたり時間: {data['total_time']/len(data['predictions']):.3f}秒")
    ...
    if 'performance_tests' in results:
        ...
        if 'batch_processing' in perf_tests and 'error' not in perf_tests['batch_processing']:
