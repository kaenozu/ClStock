        ...
        for category, stocks in categories.items():
            ...
            if category_errors:
                ...
                category_mae = np.mean(
                    np.abs(np.array(category_predictions) - np.array(category_actuals))
                )
                ...
                print(
                    f"  テスト数: {len(category_predictions)} (有効MAPE: {len(category_errors)})"
                )

        return category_results

    def _specialized_predict_with_data(
