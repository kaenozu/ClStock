...
class SocialMediaAnalyzer:
    ...
    def analyze_social_sentiment(
        ...
        for post in social_posts:
            ...
            # センチメント分析
            bullish_count = sum(
                1 for expr in self.bullish_expressions if expr in text.lower()
            )
            bearish_count = sum(
                1 for expr in self.bearish_expressions if expr in text.lower()
            )
...
class MarketSentimentAnalyzer:
    ...
    def __init__(self, history_window: int = 100):
        ...
        # センチメント重み設定
        self.weights = {"news": 0.3, "social": 0.2, "technical": 0.5}
    ...
    def analyze_comprehensive_sentiment(
        ...
        if social_data:
            social_sentiment, volume_indicator = (
                self.social_analyzer.analyze_social_sentiment(social_data)
            )
    ...
    def _calculate_sentiment_momentum(
        ...
        """センチメントモメンタム計算"""
        if (
            symbol not in self.sentiment_history
            or len(self.sentiment_history[symbol]) < 2
        ):
            return 0.0

        recent_sentiments = [
            s.sentiment_score for s in self.sentiment_history[symbol][-10:]
        ]
        ...
        if len(recent_sentiments) >= 3:
            ...
            previous_ma = (
                np.mean(recent_sentiments[-6:-3])
                if len(recent_sentiments) >= 6
                else recent_sentiments[0]
            )
    ...
    def _update_history(self, symbol: str, sentiment_data: SentimentData):
        ...
        if len(self.sentiment_history[symbol]) > self.history_window:
            self.sentiment_history[symbol] = self.sentiment_history[symbol][
                -self.history_window :
            ]
    ...
    def get_sentiment_trend(self, symbol: str, periods: int = 20) -> Dict[str, Any]:
        ...
        if (
            ...
            return {"trend": "neutral", "strength": 0.0, "turning_points": []}
    ...
    def detect_sentiment_anomaly(self, symbol: str) -> Dict[str, Any]:
        ...
        if (
            symbol not in self.sentiment_history
            or len(self.sentiment_history[symbol]) < 20
        ):
            return {"anomaly_detected": False}

        recent_sentiments = [
            s.sentiment_score for s in self.sentiment_history[symbol][-20:]
        ]
