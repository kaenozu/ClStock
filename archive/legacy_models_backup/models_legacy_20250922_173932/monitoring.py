...
class ModelPerformanceMonitor:
    ...
    def record_prediction(self, model_name: str, symbol: str, prediction: float,
                         actual: Optional[float] = None, confidence: float = 0.5):
        """予測結果を記録"""
    ...
    def _calculate_and_record_accuracy(self, model_name: str, prediction: float, actual: float):
        """精度を計算して記録"""
        # 方向精度（上昇/下降の予測精度）
        ...
        for metric in accuracy_metrics:
            ...
            if len(self.performance_history[metric.name]) > self.max_history:
                self.performance_history[metric.name] = self.performance_history[metric.name][-self.max_history:]

    def set_performance_threshold(self, metric_name: str, min_value: Optional[float] = None,
    ...
    def detect_performance_anomalies(self, model_name: str, window_size: int = 20) -> List[Dict[str, Any]]:
        """パフォーマンス異常を検出"""
        anomalies = []
    ...
    def get_model_comparison(self, models: List[str], metric_type: str = 'direction_accuracy') -> Dict[str, Any]:
        """モデル間のパフォーマンス比較"""
        comparison = {
    ...
    def get_monitoring_dashboard_data(self) -> Dict[str, Any]:
        ...
        for model in models:
            ...
            dashboard['model_status'][model] = {
                ...
                'recent_accuracy': (
                    summary['metrics'].get(f'{model}_direction_accuracy', {}).get('accuracy_percentage', 0)
                ),
                'prediction_count': (
