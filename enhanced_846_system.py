...
class Enhanced846System:
    ...
    def create_features_846_enhanced(self, data):
        ...
        features['trend_persistence'] = (
            (features['ma_bullish'] & (features['sma10_slope'] > 0)).astype(int) +
            (features['ma_bearish'] & (features['sma10_slope'] < 0)).astype(int)
        )
        ...
        # 3. RSI勢い（保守的）
        features['rsi_strength'] = (
            (rsi > 60).astype(int) - (rsi < 40).astype(int)
        )
    ...
    def create_enhanced_ensemble(self):
        ...
        models = [
            ...
            ('lr_enhanced', LogisticRegression(
                random_state=123, max_iter=300, C=0.8
            )),

            ('rf_conservative', RandomForestClassifier(
                n_estimators=50, max_depth=6, min_samples_split=8,
    ...
    def test_enhanced_846(self, symbols):
        ...
        for symbol in symbols[:20]:  # まず20銘柄でテスト
            try:
                ...
                if high_conf_acc > 0:
                    print(f"  高信頼度: {high_conf_acc:.1%} ({high_conf_mask.sum()}サンプル)")
    ...
    def _analyze_results(self, results, success_846_count, breakthrough_count):
        ...
        for i, (symbol, result) in enumerate(sorted_results[:5], 1):
            status = "🚀 BREAKTHROUGH" if result['accuracy'] > 0.846 else "⭐ TARGET" if result['accuracy'] >= 0.846 else "○ GOOD"
