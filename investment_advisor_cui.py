...
class InvestmentAdvisorCUI:
    ...
    def __init__(
        ...
        medium_change_weak_negative=-2,
    ):  # 1ヶ月で2%以上の下落（弱い変化、マイナス）
    ...
    def _integrate_recommendations(self, short: Dict, medium: Dict) -> Dict[str, Any]:
        ...
        # 統合判定ロジック（現実的な閾値に調整）
        if (
            short_change > self.thresholds["short_change_strong"]
            and medium_change > self.thresholds["medium_change_strong"]
        ):
            action = "強い買い"
            timing = f"【即座】{buy_date.strftime('%m/%d')}寄り付きで買い → {sell_date.strftime('%m/%d')}頃売却"
            confidence = (short_confidence + medium_confidence) / 2
        elif (
            short_change > self.thresholds["short_change_moderate"]
            and medium_change > self.thresholds["medium_change_moderate"]
        ):
            action = "買い"
            timing = f"【今週中】{buy_date.strftime('%m/%d')}～{next_week.strftime('%m/%d')}に買い → {sell_date.strftime('%m/%d')}頃売却"
            confidence = (short_confidence + medium_confidence) / 2
        elif (
            short_change < self.thresholds["short_change_strong_negative"]
            and medium_change < self.thresholds["medium_change_strong_negative"]
        ):
            action = "強い売り"
            timing = f"【即座】{buy_date.strftime('%m/%d')}寄り付きで売り → {sell_date.strftime('%m/%d')}まで避難"
            confidence = (short_confidence + medium_confidence) / 2
        elif (
            short_change < self.thresholds["short_change_weak"]
            and medium_change < self.thresholds["medium_change_moderate_negative"]
        ):
            action = "売り"
    ...
    def get_top_recommendations(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for i, symbol in enumerate(self.target_symbols[:limit], 1):
            name = self.symbol_names.get(self.target_universe.to_base(symbol), symbol)
            print(f"分析進行: {i}/{min(limit, len(self.target_symbols))} - {name}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="短期強い変化閾値 (default: 0.5)",
    )
    parser.add_argument(
        "--short-change-moderate",
        type=float,
        default=0.2,
        help="短期中程度変化閾値 (default: 0.2)",
    )
    parser.add_argument(
        "--short-change-weak",
        type=float,
        default=-0.2,
        help="短期弱い変化閾値 (default: -0.2)",
    )
    parser.add_argument(
        "--short-change-strong-negative",
        type=float,
        default=-0.5,
        help="短期強い変化閾値 (負) (default: -0.5)",
    )
    parser.add_argument(
        "--medium-change-strong",
        type=float,
        default=4,
        help="中期強い変化閾値 (default: 4)",
    )
    parser.add_argument(
        "--medium-change-moderate",
        type=float,
        default=2,
        help="中期中程度変化閾値 (default: 2)",
    )
    parser.add_argument(
        "--medium-change-weak",
        type=float,
        default=1.5,
        help="中期弱い変化閾値 (default: 1.5)",
    )
    parser.add_argument(
        "--medium-change-strong-negative",
        type=float,
        default=-4,
        help="中期強い変化閾値 (負) (default: -4)",
    )
    parser.add_argument(
        "--medium-change-moderate-negative",
        type=float,
        default=-2,
        help="中期中程度変化閾値 (負) (default: -2)",
    )
    parser.add_argument(
        "--medium-change-weak-negative",
        type=float,
        default=-1.5,
        help="中期弱い変化閾値 (負) (default: -1.5)",
    )
