...
class UltraOptimizedInvestmentSystem(BaseInvestmentSystem):
    ...
    def identify_ultra_opportunity(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(f"ウルトラ分析エラー {symbol}: {str(e)}")
            return None
    
    def _perform_ultra_analysis(self, symbol: str, stock_data: pd.DataFrame) -> Dict[str, Any]:
        """ウルトラ級分析実行"""
        ...
        # 高度技術分析
        technical_indicators = self._calculate_technical_indicators(close, high, low, volume)
    ...
    def _calculate_technical_indicators(self, close: pd.Series, high: pd.Series, 
        ...
        return {
            ...
            'bb_position': self._calculate_bb_position(close.iloc[-1], bb_upper.iloc[-1], bb_lower.iloc[-1]),
            'macd_signal_strength': self._calculate_macd_strength(macd_line, macd_signal),
            'rsi': rsi.iloc[-1] if not rsi.empty else 50,
            'stoch_momentum': (stoch_k.iloc[-1] + stoch_d.iloc[-1]) / 2 if not stoch_k.empty else 50,
            'trend_strength': adx.iloc[-1] if not adx.empty else 25,
            'composite_score': composite_score,
            'entry_signal': entry_signal
        }
    
    def _calculate_bollinger_bands(self, close: pd.Series) -> Tuple[pd.Series, pd.Series]:
        """ボリンジャーバンド計算"""
    ...
    def _perform_momentum_analysis(self, close: pd.Series) -> Dict[str, float]:
        ...
        if momentum_scores:
            ...
            momentum_consistency = 1 - (np.std(momentum_scores) / (abs(np.mean(momentum_scores)) + 1))
        else:
    ...
    def _analyze_volume_profile(self, close: pd.Series, volume: pd.Series) -> Dict[str, float]:
        """出来高プロファイル分析"""
        if len(volume) < 20:
            return {'volume_strength': 50, 'volume_trend': 0, 'price_volume_correlation': 0}
        
        # 出来高トレンド
        volume_ma_short = volume.rolling(5).mean()
        volume_ma_long = volume.rolling(20).mean()
        volume_trend = ((volume_ma_short.iloc[-1] - volume_ma_long.iloc[-1]) / volume_ma_long.iloc[-1]) * 100
        ...
        return {
            ...
            'price_volume_correlation': correlation * 100 if not np.isnan(correlation) else 0
        }
    
    def _analyze_volatility(self, close: pd.Series) -> Dict[str, float]:
        """ボラティリティ分析"""
        if len(close) < 20:
            return {'current_volatility': 0, 'volatility_trend': 0, 'optimal_volatility': 50}
        ...
        vol_long = returns.tail(30).std() * np.sqrt(252) * 100 if len(returns) >= 30 else current_vol
    ...
    def _calculate_ultra_score(self, technical: Dict, momentum: Dict, 
                              volume: Dict, volatility: Dict) -> float:
        """ウルトラスコア計算"""
    ...
    def _meets_ultra_criteria(self, opportunity: Dict[str, Any]) -> bool:
        """ウルトラ基準の判定"""
        return (opportunity['ultra_score'] >= ULTRA_MIN_SCORE and
                opportunity['confidence'] >= ULTRA_MIN_CONFIDENCE and
    ...
    def calculate_ultra_position_size(self, opportunity: Dict[str, Any]) -> int:
        """ウルトラポジションサイズ計算"""
        base_amount = self._calculate_position_size(self.current_capital, opportunity['confidence'])
    ...
    def execute_ultra_trade(self, opportunity: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logging.error(f"ウルトラ取引実行エラー {symbol}: {str(e)}")
            return False
    ...
    def run_ultra_backtest(self) -> Dict[str, Any]:
        ...
        risk_distribution = {'低リスク': 0, '中リスク': 0, '高リスク': 0, '超高リスク': 0}
        ...
        # ウルトラ結果表示
        self._display_ultra_backtest_results(results, ultra_opportunities, 
                                           successful_ultra_trades, risk_distribution)
    ...
    def _simulate_ultra_position_management(self):
        ...
        for symbol in list(self.positions.keys()):
            try:
                ...
                # 段階的利確戦略
                self._execute_staged_profit_taking(symbol, stock_data, base_holding_days)
                
            except Exception as e:
                logging.warning(f"ウルトラポジション管理エラー {symbol}: {str(e)}")
    
    def _execute_staged_profit_taking(self, symbol: str, stock_data: pd.DataFrame, holding_days: int):
        """段階的利確実行"""
            ...
            if len(stock_data) > holding_days + i * 10:
                ...
                if profit_rate >= target and remaining_shares > 0:
                    ...
                    if sell_shares > 0:
                        ...
                        print(f"  [上昇] {symbol} 段階利確{i+1}: {profit_rate*100:+.1f}% "
    ...
    def _display_ultra_backtest_results(self, results: Dict[str, Any], opportunities: int, 
                                       successful_trades: int, risk_distribution: Dict[str, int]):
        """ウルトラバックテスト結果表示"""
        print(f"\n" + "=" * 70)
        print("ウルトラバックテスト結果")
        print("=" * 70)
    ...
    def _calculate_macd_strength(self, macd_line: pd.Series, macd_signal: pd.Series) -> float:
        """MACDシグナル強度計算"""
    ...
    def _determine_entry_signal(self, close: pd.Series, sma_20: pd.Series, sma_50: pd.Series,
                               bb_upper: pd.Series, bb_lower: pd.Series, 
                               macd_line: pd.Series, macd_signal: pd.Series, rsi: pd.Series) -> bool:
        """エントリーシグナル判定"""
        if any(s.empty for s in [sma_20, sma_50, bb_upper, bb_lower, macd_line, macd_signal, rsi]):
            return False
    ...
    def _calculate_technical_composite_score(self, sma_20: pd.Series, sma_50: pd.Series,
                                           rsi: pd.Series, macd_line: pd.Series, 
                                           macd_signal: pd.Series, stoch_k: pd.Series, 
                                           adx: pd.Series) -> float:
        """技術指標複合スコア計算"""
        scores = []
