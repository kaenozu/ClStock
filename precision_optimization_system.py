...
class PrecisionOptimizationSystem:
    ...
    def test_integrated_system_precision(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:25]:
            try:
                ...
                # 過去データでの検証
                validation_result = self._validate_prediction_accuracy(symbol, prediction)
    ...
    def _validate_trend_following_accuracy(self, data: pd.DataFrame, symbol: str) -> Dict:
        """トレンドフォロー予測の検証"""
        close = data['Close']
        ...
        for i in range(50, len(data) - 5, 5):  # 5日おきにテスト
            ...
            # 予測実行（過去時点）
            try:
                features = self.predictor._create_trend_direction_features(historical_data.iloc[-30:])
                direction_pred = self.predictor._calculate_trend_direction(features, historical_data)
    ...
    def _analyze_precision_results(self, results: List[Dict]) -> Dict:
        ...
        if strong_trend_results:
            ...
            # 高信頼度サンプルの分析
            high_conf_results = [r for r in strong_trend_results if r['confidence'] > 0.7]
            if high_conf_results:
                high_conf_accuracies = [r['validation_accuracy'] for r in high_conf_results]
                print(f"  高信頼度(>70%)精度: {np.mean(high_conf_accuracies):.1%} ({len(high_conf_results)}銘柄)")

            # 90%以上達成
            elite_results = [r for r in strong_trend_results if r['validation_accuracy'] >= 0.9]
        ...
        if elite_all:
            print(f"\nエリート銘柄 (85%以上):")
            for r in sorted(elite_all, key=lambda x: x['validation_accuracy'], reverse=True):
                trend_mark = "🔥" if r['is_strong_trend'] else "📈"
                print(f"  {r['symbol']}: {r['validation_accuracy']:.1%} {trend_mark} "
    ...
    def _test_parameter_combination(self, symbols: List[str], trend_th: float,
                                  conf_th: float, cons_days: int) -> float:
        """パラメータ組み合わせのテスト"""
        total_correct = 0
        total_tests = 0
        ...
        for symbol in symbols:
            try:
                ...
                for i in range(30, len(data) - 3, 5):
                    # 簡易テスト
                    recent_trend = (close.iloc[i] - close.iloc[i-cons_days]) / close.iloc[i-cons_days]
