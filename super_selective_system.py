...
class BaseInvestmentSystem:
    ...
    def _execute_trade_order(self, symbol: str, action: str, shares: int, price: float, date):
        """取引実行の共通メソッド"""
        if action == 'BUY':
    ...
    def _record_transaction(self, action: str, symbol: str, shares: int, price: float, date, profit: float = 0):
        """取引記録"""
        transaction = {
            'action': action,
            'symbol': symbol,
    ...
    def _should_sell_position(self, symbol: str, current_price: float) -> bool:
        ...
        return change_rate >= self.min_profit_threshold or change_rate <= self.max_loss_threshold
...
class SuperSelectiveInvestmentSystem(BaseInvestmentSystem):
    ...
    def _analyze_opportunity(self, symbol: str, stock_data: pd.DataFrame) -> Dict[str, Any]:
        """機会分析"""
    ...
    def _meets_super_criteria(self, opportunity: Dict[str, Any]) -> bool:
        """超基準の判定"""
        return (opportunity['total_score'] >= SUPER_MIN_SCORE and
                opportunity['confidence'] >= SUPER_MIN_CONFIDENCE)
            ...
            try:
                ...
                if len(stock_data) > holding_days:
                    ...
                    if self._should_sell_position(symbol, sell_price):
                        ...
                        print(f"  [売却] {symbol} 売却: {profit_rate:+.2f}% ({profit:+,.0f}円)")
                
            except Exception as e:
                logging.warning(f"ポジション管理エラー {symbol}: {str(e)}")
    
    def _display_backtest_results(self, results: Dict[str, Any], opportunities: int, successful_trades: int):
        """バックテスト結果表示"""
        print(f"\n" + "=" * 60)
        print("バックテスト結果")
        print("=" * 60)
