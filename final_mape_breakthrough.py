...
class FinalMAPEBreakthrough:
    ...
    def ultra_conservative_predict(self, symbol: str) -> float:
        ...
        try:
            ...
            # 予測幅を極限まで小さく
            max_prediction = min(0.002, recent_vol * 0.1)  # 0.2%または極小ボラティリティ
            ...
            logger.error(f"Error in ultra conservative prediction for {symbol}: {str(e)}")
            return 0.0
    ...
    def momentum_reversal_predict(self, symbol: str) -> float:
        ...
        try:
            ...
            logger.error(f"Error in momentum reversal prediction for {symbol}: {str(e)}")
            return 0.0
        ...
        for method_name, method_func in methods.items():
            ...
            for symbol in symbols[:5]:
                try:
                    ...
                    logger.warning(f"Error testing {symbol} with {method_name}: {str(e)}")
                    continue
            ...
            if predictions:
                ...
                # 従来のMAPE（有効ケースのみ）
                traditional_mape = np.mean(valid_errors) if valid_errors else float('inf')
                ...
                print(f"  予測統計: 平均{np.mean(predictions_arr):.4f}, 標準偏差{np.std(predictions_arr):.4f}")
    ...
    def _predict_with_historical_data(self, data: pd.DataFrame, method: str) -> float:
        ...
        try:
            ...
            if method == 'ultra_conservative':
                # 極保守的
                recent_mean = returns.iloc[-3:].mean() if len(returns) >= 3 else returns.mean()
                ...
                # アンサンブル
                conservative = returns.iloc[-3:].mean() * 0.05 if len(returns) >= 3 else 0
                reversal = -returns.iloc[-1] * 0.15 if abs(returns.iloc[-1]) > returns.std() * 0.5 else returns.iloc[-1] * 0.05
                zero = 0.0
                return np.average([conservative, reversal, zero], weights=[0.3, 0.2, 0.5])
...
def main():
    ...
    for method, metrics in results.items():
        print(f"{method}:")
        print(f"  最良MAPE: {min(metrics['smart_mape'], metrics['traditional_mape']):.2f}%")
