...
class CorrectMAPEPredictor:
    ...
    def create_target_variable(self, data: pd.DataFrame, prediction_days: int = 7) -> pd.Series:
        """目標変数作成（中期リターン予測）"""
        # 7日後の累積リターン率
    ...
    def train_models(self, symbols: List[str]) -> Dict:
        ...
        print(f"総訓練データ: {X_combined.shape[0]}サンプル, {X_combined.shape[1]}特徴量")
    ...
    def predict_return(self, symbol: str) -> float:
        ...
        try:
            ...
            # 最新データ
            latest_features = features.select_dtypes(include=[np.number]).iloc[-1:].fillna(0)
        ...
        for symbol in symbols[:5]:
            try:
                ...
                for i in range(50, 10, -7):  # 7日ずつ
                    ...
                    end_price = data.iloc[-i+7]['Close'] if i >= 7 else data.iloc[-1]['Close']
    ...
    def _simple_ml_predict(self, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            momentum = (data['Close'].iloc[-1] - data['Close'].iloc[-7]) / data['Close'].iloc[-7]
...
def main():
    ...
    if 'error' not in train_results:
        ...
        for symbol in symbols[:5]:
            ...
            print(f"{symbol}: 7日後リターン予測 {pred_return:.3f} ({pred_return*100:.1f}%)")
