...
class AdvancedRiskManager:
    ...
    def calculate_dynamic_stop_loss(self, symbol: str, entry_price: float,
                                  confidence: float, volatility: float) -> dict:
        """動的ストップロス計算"""
        ...
        # 1. 信頼度による調整
        confidence_adjustment = 1.0 - (confidence - 0.5) * 0.5  # 信頼度が高いほど損切り緩く
        ...
        # 最終的な動的損切り率
        dynamic_stop_loss = base_stop_loss * confidence_adjustment * vol_adjustment * pattern_adjustment
    ...
    def calculate_portfolio_var(self, confidence_level: float = 0.05,
        ...
        try:
            ...
            weights = np.array([position_values.get(symbol, 0) / total_value
    ...
    def calculate_position_sizing_kelly(self, win_probability: float,
                                      avg_win: float, avg_loss: float,
                                      current_capital: float) -> dict:
    ...
    def assess_maximum_drawdown_risk(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                "recommendation": self._get_drawdown_recommendation(current_drawdown_pct)
            }
    ...
    def _generate_risk_recommendations(self, risk_score: float, var_result: dict,
                                     correlation_result: dict, drawdown_result: dict) -> list:
        """リスク状況に基づく推奨アクション"""
        recommendations = []
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        print(f"リスク評価時刻: {risk_report['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        print(f"ポートフォリオボラティリティ: {var_data.get('portfolio_volatility', 0):.1%}")
...
class TestAdvancedRiskManager:
    ...
    def test_kelly_criterion_calculation(self):
        ...
        for i, (win_prob, avg_win, avg_loss, should_be_positive) in enumerate(test_cases, 1):
    ...
    def test_drawdown_assessment(self):
        ...
        try:
            ...
            print(f"ドローダウン評価: {'OK' if has_drawdown and has_risk_level else 'NG'}")
    ...
    def test_comprehensive_risk_report(self):
        ...
        try:
            ...
            required_keys = [
                'timestamp', 'overall_risk_score', 'overall_risk_level',
                'var_analysis', 'correlation_analysis', 'drawdown_analysis',
                'recommendations'
            ]
            ...
            print(f"総合レポート: {'OK' if has_all_keys and has_recommendations else 'NG'}")
