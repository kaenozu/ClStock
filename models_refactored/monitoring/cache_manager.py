...
class AdvancedCacheManager(CacheProvider):
    """高度なキャッシュ管理システム"""

    def __init__(self, max_cache_size: int = 1000, ttl_hours: int = 24, cleanup_interval: int = 1800):
        self.feature_cache = {}
        ...
        self.cleanup_interval = cleanup_interval  # 30分ごとにクリーンアップ（デフォルト）
    ...
    def cleanup_old_cache(self):
        ...
        total_removed = removed_feature + removed_prediction + feature_removed_lru + prediction_removed_lru
        if total_removed > 0:
            self._update_cache_stats()
            logger.debug(f"Cache cleanup completed. Removed {removed_feature} expired features, "
    ...
    def load_cache_from_disk(self) -> bool:
        ...
        try:
            ...
            self.feature_cache = self._deserialize_cache(cache_data.get("feature_cache", {}))
            self.prediction_cache = self._deserialize_cache(cache_data.get("prediction_cache", {}))
...
class RealTimeCacheManager(AdvancedCacheManager):
    ...
    def __init__(self, max_cache_size: int = 5000, ttl_hours: int = 1, cleanup_interval: int = 600):
        ...
        # 自動クリーンアップ設定（リアルタイム用）
        self.realtime_cleanup_interval = cleanup_interval  # 10分ごとにクリーンアップ（デフォルト）
        self._start_realtime_cleanup_thread()
        
        logger.info(f"RealTimeCacheManager initialized with cleanup_interval={cleanup_interval}s, max_cache_size={max_cache_size}")

    def _start_realtime_cleanup_thread(self):
        """リアルタイムキャッシュ用自動クリーンアップスレッドを開始"""
        if self.auto_cleanup_enabled:
            realtime_cleanup_thread = threading.Thread(target=self._realtime_cleanup_worker, daemon=True)
    ...
    def _realtime_cleanup_worker(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                self.cleanup_real_time_cache(older_than_hours=2)  # 2時間以上古いデータをクリーンアップ
                logger.debug(f"Realtime cache cleanup completed. Tick cache symbols: {len(self.tick_cache)}, Order book cache symbols: {len(self.order_book_cache)}")
    ...
    def cache_order_book_data(self, order_book_data, max_history: Optional[int] = None) -> None:
        """板情報データを時系列キャッシュに保存"""
    ...
    def get_tick_history(self, symbol: str, 
    ...
    def get_order_book_history(self, symbol: str,
                              start_time: Optional[datetime] = None,
                              end_time: Optional[datetime] = None,
                              limit: Optional[int] = None) -> List:
        """指定期間の板情報履歴を取得"""
    ...
    def calculate_market_metrics(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_update": tick_history[-1].timestamp.isoformat() if tick_history else None
        }
    ...
    def _update_real_time_stats(self, timestamp: datetime) -> None:
        """リアルタイム統計を更新"""
        if (self.real_time_stats["oldest_data_timestamp"] is None or 
            timestamp < self.real_time_stats["oldest_data_timestamp"]):
            self.real_time_stats["oldest_data_timestamp"] = timestamp
            
        if (self.real_time_stats["newest_data_timestamp"] is None or 
            timestamp > self.real_time_stats["newest_data_timestamp"]):
    ...
    def get_real_time_cache_stats(self) -> Dict[str, Any]:
        ...
        base_stats.update({
            ...
            "cache_sizes": {
                "tick_cache_entries": sum(len(entries) for entries in self.tick_cache.values()),
                "order_book_cache_entries": sum(len(entries) for entries in self.order_book_cache.values())
            }
        })
    ...
    def cleanup_real_time_cache(self, older_than_hours: int = 24) -> int:
        ...
        for symbol in list(self.tick_cache.keys()):
            ...
            self.tick_cache[symbol] = [
                entry for entry in self.tick_cache[symbol]
        ...
        for symbol in list(self.order_book_cache.keys()):
            ...
            self.order_book_cache[symbol] = [
                entry for entry in self.order_book_cache[symbol]
                if entry["timestamp"] > cutoff_time
            ]
