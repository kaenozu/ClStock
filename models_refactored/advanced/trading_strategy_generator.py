...
class StrategyGenerator:
    ...
    def generate_momentum_strategy(self, symbol: str, price_data: pd.DataFrame,
                                 parameters: Dict[str, Any] = None) -> TradingStrategy:
        """モメンタム戦略生成"""
    ...
    def generate_mean_reversion_strategy(self, symbol: str, price_data: pd.DataFrame,
                                       parameters: Dict[str, Any] = None) -> TradingStrategy:
        """平均回帰戦略生成"""
        if parameters is None:
            parameters = {
    ...
    def generate_breakout_strategy(self, symbol: str, price_data: pd.DataFrame,
                                 parameters: Dict[str, Any] = None) -> TradingStrategy:
        """ブレイクアウト戦略生成"""
        if parameters is None:
            parameters = {
    ...
    def _calculate_strategy_performance(self, price_data: pd.DataFrame,
                                      strategy_type: StrategyType,
        ...
        try:
            ...
            return {
                'expected_return': min(max(base_return, -0.5), 2.0),  # -50%から200%に制限
...
class SignalGenerator:
    ...
    def generate_signals(self, symbol: str, price_data: pd.DataFrame,
                        ...
                        sentiment_data: Optional[Dict[str, Any]] = None) -> List[TradingSignal]:
        """取引シグナル生成"""
        signals = []
        ...
        try:
            ...
            if strategy.strategy_type == StrategyType.MOMENTUM:
                signal = self._generate_momentum_signal(symbol, price_data, strategy, current_price)
            elif strategy.strategy_type == StrategyType.MEAN_REVERSION:
                signal = self._generate_mean_reversion_signal(symbol, price_data, strategy, current_price)
            elif strategy.strategy_type == StrategyType.BREAKOUT:
                signal = self._generate_breakout_signal(symbol, price_data, strategy, current_price)
            else:
                signal = None
    ...
    def _generate_momentum_signal(self, symbol: str, price_data: pd.DataFrame,
                                strategy: TradingStrategy, current_price: float) -> Optional[TradingSignal]:
        """モメンタムシグナル生成"""
        params = strategy.parameters

        # 移動平均計算
        short_ma = self.indicators.calculate_sma(price_data['Close'], params['short_ma_period']).iloc[-1]
        long_ma = self.indicators.calculate_sma(price_data['Close'], params['long_ma_period']).iloc[-1]

        # RSI計算
        rsi = self.indicators.calculate_rsi(price_data['Close'], params['rsi_period']).iloc[-1]

        # 出来高チェック
        avg_volume = price_data['Volume'].rolling(window=20).mean().iloc[-1] if 'Volume' in price_data.columns else 1
        ...
        if (short_ma > long_ma and
            params['rsi_oversold'] < rsi < params['rsi_overbought'] and
            volume_ratio > 1.2):
            ...
            return TradingSignal(
                ...
                stop_loss=current_price * (1 - strategy.risk_management['stop_loss_pct']),
                take_profit=current_price * (1 + strategy.risk_management['take_profit_pct']),
    ...
    def _generate_mean_reversion_signal(self, symbol: str, price_data: pd.DataFrame,
                                      strategy: TradingStrategy, current_price: float) -> Optional[TradingSignal]:
        """平均回帰シグナル生成"""
        params = strategy.parameters
    ...
    def _generate_breakout_signal(self, symbol: str, price_data: pd.DataFrame,
                                strategy: TradingStrategy, current_price: float) -> Optional[TradingSignal]:
        """ブレイクアウトシグナル生成"""
        params = strategy.parameters

        # 過去20日の最高値
        resistance_level = price_data['High'].rolling(window=params['resistance_period']).max().iloc[-1]
        ...
        if (current_price > breakout_threshold and
            ...
            confidence = min(0.9, (current_price - resistance_level) / resistance_level * 10 + 0.5)
    ...
    def _adjust_signal_with_sentiment(self, signal: TradingSignal,
                                    sentiment_data: Dict[str, Any]) -> TradingSignal:
        """センチメントに基づくシグナル調整"""
        ...
        if signal.action == ActionType.BUY and sentiment_score > 0.2:
            ...
            signal.confidence = min(0.95, signal.confidence + abs(sentiment_score) * 0.2)
...
class AutoTradingStrategyGenerator:
    ...
    def generate_comprehensive_strategy(self, symbol: str, price_data: pd.DataFrame,
                                       strategy_types: Optional[List[StrategyType]] = None) -> List[TradingStrategy]:
        """包括的戦略生成"""
        if strategy_types is None:
            strategy_types = [StrategyType.MOMENTUM, StrategyType.MEAN_REVERSION, StrategyType.BREAKOUT]
        ...
        for strategy_type in strategy_types:
            try:
                if strategy_type == StrategyType.MOMENTUM:
                    strategy = self.strategy_generator.generate_momentum_strategy(symbol, price_data)
                elif strategy_type == StrategyType.MEAN_REVERSION:
                    strategy = self.strategy_generator.generate_mean_reversion_strategy(symbol, price_data)
                elif strategy_type == StrategyType.BREAKOUT:
                    strategy = self.strategy_generator.generate_breakout_strategy(symbol, price_data)
                else:
                    continue
                ...
                self.logger.error(f"Strategy generation failed for {strategy_type}: {str(e)}")

        return strategies

    def generate_trading_signals(self, symbol: str, price_data: pd.DataFrame,
                               sentiment_data: Optional[Dict[str, Any]] = None) -> List[TradingSignal]:
        """取引シグナル生成"""
        all_signals = []

        # 該当銘柄の戦略を取得
        symbol_strategies = [s for key, s in self.strategies.items() if key.startswith(symbol)]
    ...
    def _adjust_strategy_parameters(self, strategy: TradingStrategy, adjustment_type: str):
        """戦略パラメータ調整"""
        if strategy.strategy_type == StrategyType.MOMENTUM:
            if adjustment_type == 'underperform':
                # より保守的に
                strategy.parameters['rsi_overbought'] = max(60, strategy.parameters['rsi_overbought'] - 5)
                strategy.parameters['rsi_oversold'] = min(40, strategy.parameters['rsi_oversold'] + 5)
            else:
                # より積極的に
                strategy.parameters['rsi_overbought'] = min(80, strategy.parameters['rsi_overbought'] + 5)
                strategy.parameters['rsi_oversold'] = max(20, strategy.parameters['rsi_oversold'] - 5)

        elif strategy.strategy_type == StrategyType.MEAN_REVERSION:
            if adjustment_type == 'underperform':
                # より厳しい条件
                strategy.parameters['oversold_threshold'] = max(20, strategy.parameters['oversold_threshold'] - 5)
                strategy.parameters['bollinger_std'] = min(2.5, strategy.parameters['bollinger_std'] + 0.2)
            else:
                # より緩い条件
                strategy.parameters['oversold_threshold'] = min(35, strategy.parameters['oversold_threshold'] + 5)
                strategy.parameters['bollinger_std'] = max(1.5, strategy.parameters['bollinger_std'] - 0.2)
    ...
    def get_strategy_performance(self, symbol: str) -> Dict[str, Any]:
        ...
        performance_summary = {
            ...
            'avg_expected_return': np.mean([s.expected_return for s in symbol_strategies]),
    ...
    def generate_strategy_report(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'recommendations': self._generate_strategy_recommendations(symbol, performance)
        }

    def _generate_strategy_recommendations(self, symbol: str,
