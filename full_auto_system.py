...
from models_new.advanced.trading_strategy_generator import AutoTradingStrategyGenerator, ActionType
...
class FullAutoInvestmentSystem:
    ...
    async def run_full_auto_analysis(self) -> List[AutoRecommendation]:
        ...
        try:
            ...
            recommendations = await self._generate_auto_recommendations(optimized_symbols)
        ...
        while retry_count < max_retries:
            try:
                ...
                if need_optimization:
                    print(f"[実行] 最適化が必要です。実行中... (試行 {retry_count + 1}/{max_retries})")
                    ...
                    if optimization_result and 'optimized_portfolio' in optimization_result:
                        symbols = [stock.symbol for stock in optimization_result['optimized_portfolio']]
                        selected_symbols = symbols[:self.auto_settings['portfolio_size']]
                        ...
                        if len(selected_symbols) >= 5:  # 最低5銘柄は必要
                            ...
                            self.logger.info(f"✅ 最適化完了: {len(selected_symbols)}銘柄選出")
                            return selected_symbols
                        else:
                            raise Exception(f"選出銘柄数不足: {len(selected_symbols)}銘柄")
                    else:
                ...
                retry_count += 1
                error_msg = str(e)
                print(f"[エラー] 最適化エラー (試行 {retry_count}/{max_retries}): {error_msg}")
                self.logger.warning(f"TSE4000最適化エラー (試行 {retry_count}): {error_msg}")
    ...
    async def _run_tse_optimization_async(self):
        ...
        return await loop.run_in_executor(None, self.tse_optimizer.run_comprehensive_optimization)
    ...
    def _check_market_hours(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if weekday >= 5:  # 土曜日・日曜日
                ...
                return False, f"市場休業日です。次回開場: {next_monday.strftime('%m/%d（月）9:00')}"
            ...
            if morning_start <= current_minutes <= morning_end:
                ...
                if next_day.weekday() >= 5:  # 翌日が土日
                    ...
                    return False, f"市場終了後です。次回開場: {next_monday.strftime('%m/%d（月）9:00')}"
                else:
                    return False, f"市場終了後です。次回開場: {next_day.strftime('%m/%d 9:00')}"
    ...
    async def _auto_learning_and_training(self, symbols: List[str]):
        ...
        try:
            ...
            successful_learning = len([r for r in learning_results if not isinstance(r, Exception)])
    ...
    async def _learn_single_symbol(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 実時間学習システムにデータ追加
            if hasattr(self.hybrid_predictor, 'real_time_learning_enabled') and self.hybrid_predictor.real_time_learning_enabled:
                market_data = {
                    'symbol': symbol,
                    'price': price_data['Close'].iloc[-1] if 'Close' in price_data else 1000,
                    'volume': price_data['Volume'].iloc[-1] if 'Volume' in price_data else 100000,
    ...
    async def _generate_auto_recommendations(self, symbols: List[str]) -> List[AutoRecommendation]:
        """自動推奨生成"""
    ...
    async def _analyze_single_symbol(self, symbol: str) -> Optional[AutoRecommendation]:
        ...
        try:
            ...
            # 3. 戦略生成
            strategies = self.strategy_generator.generate_comprehensive_strategy(symbol, price_data)
            signals = self.strategy_generator.generate_trading_signals(
                symbol,
                price_data,
                sentiment_data={'current_sentiment': {'score': sentiment_result.sentiment_score}}
            )
            ...
            # 6. 総合判定
            if self._should_recommend(prediction_result, sentiment_result, risk_analysis):
                return self._create_recommendation(
                    symbol, current_price, prediction_result,
                    sentiment_result, buy_timing, sell_timing, risk_analysis
                )
    ...
    def _calculate_optimal_timing(self, prediction, sentiment, signals) -> Tuple[datetime, datetime]:
        """最適売買タイミング計算"""
        current_time = datetime.now()
    ...
    def _create_recommendation(self, symbol: str, current_price: float,
                             prediction, sentiment, buy_timing: datetime,
                             sell_timing: datetime, risk_analysis) -> AutoRecommendation:
        """推奨情報作成"""
