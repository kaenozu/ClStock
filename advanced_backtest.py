...
class AdvancedBacktester:
    ...
    def calculate_directional_accuracy(self, actual_returns: np.ndarray, predicted_scores: np.ndarray) -> float:
        """方向性精度計算（上昇/下降予測の正確性）"""
    ...
    def calculate_sharpe_ratio(self, returns: np.ndarray, risk_free_rate: float = 0.001) -> float:
        """シャープレシオ計算"""
        if np.std(returns) == 0:
            return 0
    ...
    def run_prediction_backtest(self, symbols: List[str],
        ...
        predictors = {
            ...
            'ml_model': StockPredictor(use_ml_model=True, ml_model_type="xgboost", use_ultra_mode=False),
            'ultra_model': StockPredictor(use_ultra_mode=True)
        }

        model_results = {name: {
            'predictions': [], 'actuals': [], 'returns': [], 'scores': []
                ...
                for i in range(len(data) - prediction_horizon):
                    ...
                    for model_name, predictor in predictors.items():
                        try:
                            ...
                            # 予測価格（スコアベース）
                            if score > 50:
                                predicted_return = (score - 50) / 50 * 0.1  # 最大10%の上昇予測
                            else:
                                predicted_return = (score - 50) / 50 * 0.1  # 最大10%の下落予測

                            model_results[model_name]['predictions'].append(predicted_return)
    ...
    def run_investment_simulation(self, symbols: List[str],
                                initial_capital: float = 1000000,
                                rebalance_frequency: int = 30) -> Dict:
        """投資シミュレーション実行"""
        ...
        for i, date in enumerate(common_dates[:-rebalance_frequency]):
            if i % rebalance_frequency == 0:  # リバランス
                ...
                # 上位3銘柄選択
                top_symbols = sorted(scores.keys(), key=lambda x: scores[x], reverse=True)[:3]
        ...
        for date in common_dates:
            ...
            for symbol in symbols:
                if symbol in all_data and date in all_data[symbol].index:
                    ...
                    current_price = all_data[symbol].loc[date, 'Close']
                    benchmark_value += equal_weight_capital * (current_price / first_price)
        ...
        benchmark_values = [b['benchmark_value'] for b in benchmark_history[:len(portfolio_history)]]

        portfolio_returns = np.diff(portfolio_values) / portfolio_values[:-1]
        benchmark_returns = np.diff(benchmark_values) / benchmark_values[:-1]

        total_return = (portfolio_values[-1] - initial_capital) / initial_capital
        benchmark_return = (benchmark_values[len(portfolio_history)-1] - initial_capital) / initial_capital
    ...
    def generate_performance_report(self, backtest_results: Dict, simulation_results: Dict):
        """性能レポート生成"""
        ...
        # テーブル形式で表示
        print(f"{'モデル':<15} {'MAPE':<8} {'方向精度':<8} {'相関係数':<8} {'サンプル数':<8}")
        print("-" * 55)
        ...
        for model_name, metrics in performance_summary.items():
            ...
            print(f"{model_name:<15} {mape_str:<8} {direction:.3f}    {correlation:.3f}    {sample_size:<8}")
