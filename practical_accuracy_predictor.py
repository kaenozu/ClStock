...
class PracticalAccuracyPredictor:
    ...
    def predict_probability_bands(self, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            if data.empty or len(data) < 10:
                return {'neutral': 1.0, 'up_small': 0.0, 'up_large': 0.0, 'down_small': 0.0, 'down_large': 0.0}
    ...
    def predict_expected_return_range(self, symbol: str) -> Tuple[float, float, float]:
        ...
        try:
            ...
            # 基本期待値
            base_expected = mean_return * 0.3 + momentum * 0.2  # 履歴とモメンタムの組み合わせ
        ...
        for symbol in symbols[:5]:
            try:
                ...
                for i in range(15, 5, -1):
                    ...
                    # 範囲予測（過去データのみ使用）
                    worst, expected, best = self._predict_range_with_data(historical_data)
    ...
    def _predict_range_with_data(self, data: pd.DataFrame) -> Tuple[float, float, float]:
        """過去データのみを使った範囲予測"""
        try:
            data = self.data_provider.calculate_technical_indicators(data)
            returns = data['Close'].pct_change().dropna()
...
def calculate_practical_metrics(results: Dict) -> Dict:
    ...
    practical_mape = (expected_mae / mean_abs_actual * 100) if mean_abs_actual > 0 else float('inf')

    # 予測の有用性（ランダム予測との比較）
    random_mae = np.mean(np.abs(actuals))  # ランダム予測（常に0予測）との比較
    improvement_ratio = (random_mae - expected_mae) / random_mae * 100 if random_mae > 0 else 0
...
def main():
    ...
    for symbol in test_symbols:
        ...
        print(f"  確率: 上昇{(probs['up_small']+probs['up_large']):.1%}, 中立{probs['neutral']:.1%}, 下降{(probs['down_small']+probs['down_large']):.1%}")
    ...
    if 'error' not in metrics:
        ...
        print(f"最終結果: 実用的MAPE {metrics['practical_mape']:.2f}%, 範囲精度 {metrics['range_accuracy']:.1f}%")
