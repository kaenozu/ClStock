...
class UltimateDirectionalSystem:
    ...
    def identify_optimal_conditions(self, data: pd.DataFrame) -> pd.Series:
        ...
        strong_trend = (
            ((sma_10 > sma_20) & (sma_20 > sma_50)) |  # 上昇トレンド
            ((sma_10 < sma_20) & (sma_20 < sma_50))    # 下降トレンド
        )

        # 2. 適度なボラティリティ
        returns = close.pct_change()
        volatility = returns.rolling(10).std()
        moderate_vol = (volatility > volatility.quantile(0.2)) & (volatility < volatility.quantile(0.8))
    ...
    def create_enhanced_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 2. 価格アクション
        features['higher_high'] = (close > close.shift(1)) & (close.shift(1) > close.shift(2))
        features['lower_low'] = (close < close.shift(1)) & (close.shift(1) < close.shift(2))
        ...
        for symbol in symbols[:25]:
            try:
                ...
                if ultra_high_accuracy > 0:
                    print(f"  超高信頼度精度: {ultra_high_accuracy:.1%} ({ultra_high_conf_mask.sum()}サンプル)")
    ...
    def _analyze_ultimate_results(self, results: List[Dict]) -> Dict:
        ...
        ultra_high_accuracies = [r['ultra_high_accuracy'] for r in results if r['ultra_high_accuracy'] > 0]
        ...
        if elite_results:
            print(f"\nエリート結果 (85%以上):")
            for result in sorted(elite_results, key=lambda x: x['accuracy'], reverse=True):
                print(f"  {result['symbol']}: {result['accuracy']:.1%} "
