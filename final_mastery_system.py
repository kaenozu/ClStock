...
class FinalMasterySystem:
    ...
    def identify_golden_trend_conditions(self, data: pd.DataFrame) -> pd.Series:
        ...
        momentum_consistent = (
            ((momentum_3d > 0.005) & strong_uptrend) |
    ...
    def create_mastery_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features['bullish_alignment'] = (sma_5 > sma_10) & (sma_10 > sma_20) & (sma_20 > sma_50)
        features['bearish_alignment'] = (sma_5 < sma_10) & (sma_10 < sma_20) & (sma_20 < sma_50)
        ...
        features['momentum_acceleration'] = features['momentum_3d'] - features['momentum_5d']
    ...
    def test_final_mastery(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:35]:  # より多くのサンプルでテスト
            try:
                ...
                if ultra_accuracy > 0:
                    print(f"  超高信頼度: {ultra_accuracy:.1%} ({ultra_conf_mask.sum()}サンプル)")
    ...
    def _analyze_mastery_results(self, all_results: List[Dict], mastery_results: List[Dict]) -> Dict:
        """マスタリー結果の分析"""
        if not all_results:
        ...
        if mastery_results:
            ...
            for r in sorted(mastery_results, key=lambda x: x['accuracy'], reverse=True):
                ultra_info = f" (超高信頼度: {r['ultra_accuracy']:.1%})" if r['ultra_accuracy'] > 0 else ""
        ...
        for threshold, label in ranges:
            ...
            print(f"  {label}: {count}/{len(all_results)} 銘柄 ({count/len(all_results)*100:.1f}%)")
