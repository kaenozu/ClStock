...
### 出力内容
- ランキング（デフォルトはTOP10、`--top`オプションで任意件数に変更可能）
- 銘柄ごとの詳細アドバイス（買い条件・購入価格・損切り・利益目標・保有期間）

## 5. システム構成
### 第1段階（CUI + API）
- **バックエンド**: Python (FastAPI)
- **CUIクライアント**: `python recommend.py`（デフォルトで上位10件。`--top`で任意件数指定可能）
- **出力形式**: JSON + テキスト（API・GUIともにCUIと同じ件数ルールを採用）
