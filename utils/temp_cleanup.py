...
class TempFileCleanup:
    ...
    def cleanup_all(self):
        ...
        if failed_files or failed_dirs:
            logger.warning(f"Failed to clean up {len(failed_files)} files and {len(failed_dirs)} directories")
        else:
        ...
        try:
            ...
            for file_path in files:
                try:
                    ...
                    logger.warning(f"Error checking/cleaning old temp file {file_path}: {e}")
                    
            if cleaned_count > 0:
                logger.info(f"Cleaned up {cleaned_count} old temporary files matching pattern: {pattern}")
