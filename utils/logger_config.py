...
class CentralizedLogger:
    ...
    def collect_service_logs(self, service_name: str, max_lines: int = 100) -> List[str]:
        """サービスのログ収集"""
        logs = []
    ...
    def write_centralized_log(self, message: str, level: str = "INFO", service: str = "SYSTEM"):
        """集約ログへの書き込み"""
    ...
    def get_recent_logs(self, hours: int = 1, service_filter: Optional[str] = None) -> List[str]:
        """最近のログ取得"""
        recent_logs = []
    ...
    def cleanup_old_logs(self, days: int = 7):
        ...
        for log_file in self.log_dir.glob("*.log"):
            try:
                ...
                self.write_centralized_log(f"ログクリーンアップ失敗 {log_file}: {e}", "WARNING")
                
        if cleaned_files:
            self.write_centralized_log(f"古いログファイル削除: {len(cleaned_files)}件", "INFO")
    ...
    def generate_log_report(self) -> Dict[str, Any]:
        ...
        # ディスク使用量
        total_size = sum(f.stat().st_size for f in self.log_dir.glob("*.log") if f.exists())
        report["disk_usage"] = {
