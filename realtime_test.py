...
import pandas as pd
from realtime_trading_system import RealTimeTradingSystem, Pattern846Detector, RealTimeDataProvider
...
def test_order_simulation():
    ...
    # シミュレーション注文
    test_orders = [
        {'symbol': '7203', 'action': 'BUY', 'size': 100, 'price': 2500, 'confidence': 0.87},
        {'symbol': '6758', 'action': 'BUY', 'size': 200, 'price': 12000, 'confidence': 0.85}
    ]
    ...
    for order in test_orders:
        try:
            ...
            if result['status'] == 'executed':
                ...
                print(f"注文成功: {order['symbol']} {order['action']} {order['size']}株")
            else:
