...
# Phase 3システムのインポート
from models_new.advanced.market_sentiment_analyzer import MarketSentimentAnalyzer, SentimentData
from models_new.advanced.prediction_dashboard import PredictionDashboard, VisualizationData
from models_new.advanced.trading_strategy_generator import AutoTradingStrategyGenerator, StrategyType
...
class Phase3IntegrationTester:
    ...
    def test_sentiment_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'recommendation': report['recommendation'] if 'recommendation' in report else 'N/A'
            }
        ...
        try:
            ...
            for symbol in symbols:
                ...
                price_data[symbol] = pd.DataFrame({
                    'Close': np.random.randn(100).cumsum() + np.random.uniform(800, 2000),
                    'High': np.random.randn(100).cumsum() + np.random.uniform(820, 2020),
                    'Low': np.random.randn(100).cumsum() + np.random.uniform(780, 1980),
                    'Volume': np.random.randint(100000, 1000000, 100)
                }, index=dates)

            # ポートフォリオリスク分析
            portfolio_risk = risk_manager.analyze_portfolio_risk(portfolio_data, price_data)
    ...
    def test_comprehensive_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. 戦略生成（センチメント考慮）
            strategies = strategy_generator.generate_comprehensive_strategy(test_symbol, price_data)
            signals = strategy_generator.generate_trading_signals(
                test_symbol,
                price_data,
