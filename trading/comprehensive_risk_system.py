...
from trading.risk_management import PortfolioRiskManager, DynamicRiskManager, RiskMetrics
...
class ComprehensiveRiskManager:
    ...
    def calculate_comprehensive_risk(self, 
                                   ...
                                   market_data: Optional[Dict] = None) -> ComprehensiveRiskAssessment:
        """
        包括的リスク評価を計算
        ...
        # 退出戦略の計算
        if market_data is not None and 'High' in market_data and 'Low' in market_data and 'Close' in market_data:
    ...
    def _calculate_risk_score(self, basic_metrics: RiskMetrics, advanced_metrics: AdvancedRiskMetrics) -> float:
        """
        総合リスクスコアを計算 (0.0-1.0)
        ...
        var_95_score = max(0.0, min(1.0, (basic_metrics.var_95 - self.risk_thresholds['var_95']) / 0.10))
        scores.append(var_95_score)
        
        var_99_score = max(0.0, min(1.0, (basic_metrics.var_99 - self.risk_thresholds['var_99']) / 0.15))
        scores.append(var_99_score)
        
        # 最大ドローダウンスコア
        max_drawdown_score = max(0.0, min(1.0, (basic_metrics.max_drawdown - self.risk_thresholds['max_drawdown']) / 0.20))
        scores.append(max_drawdown_score)
        
        # ボラティリティスコア
        vol_score = max(0.0, min(1.0, (basic_metrics.volatility - self.risk_thresholds['volatility']) / 0.20))
        ...
        # 期待ショートフォールスコア
        es_score = max(0.0, min(1.0, (advanced_metrics.expected_shortfall_95 + 0.05) / 0.15))
        scores.append(es_score)
        
        # ストレステスト損失スコア
        stress_score = max(0.0, min(1.0, (advanced_metrics.stress_test_loss + 0.05) / 0.20))
    ...
    def get_risk_adjusted_parameters(self, 
                                   ...
                                   base_take_profit_pct: float = 0.10) -> Tuple[float, float]:
        """
        ...
        volatility_factor = basic_metrics.volatility / 0.2 if basic_metrics.volatility > 0 else 1.0  # 20%を基準
        
        # 最大ドローダウンが大きい場合はより保守的に
        drawdown_factor = 1.0 - (basic_metrics.max_drawdown / -0.20) if basic_metrics.max_drawdown < 0 else 1.0  # 20%ドローダウンを基準
        drawdown_factor = max(0.5, drawdown_factor)  # 最低でも50%
        
        # 調整後のパラメータ
        adjusted_stop_loss = base_stop_loss_pct * (1 + volatility_factor * 0.5) * drawdown_factor
        adjusted_take_profit = base_take_profit_pct * (1 - volatility_factor * 0.2) / drawdown_factor
...
if __name__ == "__main__":
    ...
    if buy_result:
        ...
        if sell_result:
            print(f"売却成功: {sell_result['shares']}株, 価値: {sell_result['value']:,.0f}円")
