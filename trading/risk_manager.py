...
class DemoRiskManager:
    ...
    def can_open_position(self,
        ...
        try:
            ...
            if position_ratio > max_allowed:
                self.logger.info(f"ポジションサイズ超過: {symbol} {position_ratio:.1%} > {max_allowed:.1%}")
                return False
            ...
            if new_sector_exposure > self.risk_limits.max_sector_exposure:
                self.logger.info(f"セクター集中度超過: {sector} {new_sector_exposure:.1%}")
                return False
            ...
            # ドローダウンチェック
            if self.current_drawdown > self.risk_limits.max_drawdown * 0.8:  # 80%に達したら新規停止
    ...
    def calculate_optimal_position_size(self,
        ...
        try:
            ...
            # 最終ポジションサイズ
            adjusted_kelly = kelly_ratio * precision_multiplier * confidence_multiplier * market_multiplier

            # 上限制限
            max_position_ratio = self._calculate_dynamic_position_limit(confidence, precision)
            ...
            self.logger.info(f"最適ポジションサイズ計算: {symbol} "
    ...
    def update_risk_metrics(self,
        ...
        try:
            ...
            # ドローダウン計算
            self.current_drawdown = (self.max_capital - current_total) / self.max_capital
    ...
    def get_risk_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'risk_utilization': {
                    'drawdown_usage': self.current_drawdown / self.risk_limits.max_drawdown,
                    'max_sector_usage': max(self.sector_exposure.values()) / self.risk_limits.max_sector_exposure if self.sector_exposure else 0.0
                },
                'daily_pnl_stats': {
                    'mean': np.mean(self.daily_pnl_history) if self.daily_pnl_history else 0.0,
                    'std': np.std(self.daily_pnl_history) if self.daily_pnl_history else 0.0,
                    'min': min(self.daily_pnl_history) if self.daily_pnl_history else 0.0,
                    'max': max(self.daily_pnl_history) if self.daily_pnl_history else 0.0
                }
            }
    ...
    def _calculate_dynamic_position_limit(self,
                                        confidence: float,
        ...
        adjusted_limit = base_limit * precision_multiplier * confidence_multiplier * drawdown_multiplier
    ...
    def _check_correlation_risk(self, symbol: str, position_size: float) -> bool:
        ...
        try:
            ...
            same_sector_count = sum(
                1 for pos_symbol in self.positions.keys()
    ...
    def _check_risk_alerts(self, current_positions: Dict[str, Dict[str, Any]]):
        ...
        try:
            ...
            # 新しいアラートのみ追加
            for alert in alerts:
                if not any(existing['message'] == alert['message']
