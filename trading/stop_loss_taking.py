...
class ATRBasedStopLoss:
    ...
    def calculate_stop_loss(
        self, current_price: float, df: pd.DataFrame, direction: str = "long"
    ) -> Tuple[float, float]:
        """
        ATRベースのストップロス価格を計算
...
class TrailingStop:
    ...
    def __init__(
        self, trail_percentage: float = 0.10, trail_amount: Optional[float] = None
    ):
        """
        Args:
...
class MultipleProfitTaking:
    ...
    def __init__(
        self,
        levels: List[Tuple[float, float]] = [(0.5, 0.30), (0.3, 0.60), (0.2, 1.00)],
    ):
        """
        Args:
    ...
    def calculate_exit_prices(
        self, entry_price: float, base_target: float
...
class SmartExitStrategy:
    ...
    def __init__(
        ...
        self.atr_stop = ATRBasedStopLoss(
            atr_period=atr_period, multiplier=atr_multiplier
        )
        ...
        self.multiple_taking = (
            MultipleProfitTaking() if enable_multiple_taking else None
        )
    ...
    def calculate_exit_strategy(
        ...
        if initial_stop_distance is None:
            atr_stop_price, current_atr = self.atr_stop.calculate_stop_loss(
                entry_price, df, direction
            )
        ...
        _, take_profit_price = self.risk_reward_optimizer.calculate_optimal_levels(
            entry_price, stop_distance * self.market_volatility_factor
        )

        # トレーリングストップ設定
        trailing_enabled = True
        trailing_distance = (
            current_atr * self.atr_stop.multiplier
            if current_atr > 0
            else stop_distance * 0.5
        )
    ...
    def evaluate_exit_conditions(
        ...
        # 損切り条件
        stop_loss_hit = (
            direction == "long" and current_price <= exit_strategy.stop_loss_price
        ) or (direction == "short" and current_price >= exit_strategy.stop_loss_price)

        # 利確条件
        take_profit_hit = (
            direction == "long" and current_price >= exit_strategy.take_profit_price
        ) or (direction == "short" and current_price <= exit_strategy.take_profit_price)

        # トレーリングストップが有効な場合の評価
        if exit_strategy.trailing_stop_enabled:
            trailing_stop_price = self.trailing_stop.update_price(
                current_price, direction
            )
            trailing_stop_hit = (
                direction == "long" and current_price <= trailing_stop_price
            ) or (direction == "short" and current_price >= trailing_stop_price)
        else:
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(
        {"Close": prices, "High": high_prices, "Low": low_prices}, index=dates
    )
    ...
    print(
        f"ATRベースストップロス: エントリー価格 {entry_price:.0f}円, ストップ価格 {stop_price:.0f}円, ATR {atr_value:.2f}"
    )
    ...
    stop_price, profit_price = risk_reward.calculate_optimal_levels(
        entry_price=3000, stop_loss_distance=100
    )
    print(
        f"リスク・リワード最適化: ストップ {stop_price:.0f}円, 利確 {profit_price:.0f}円"
    )
    ...
    print(
        f"分割利確レベル: {[(f'{price:.0f}円', f'{portion:.0%}') for price, portion in exit_levels]}"
    )

    # スマート出口戦略のテスト
    smart_exit = SmartExitStrategy()
    exit_strategy = smart_exit.calculate_exit_strategy(
        entry_price=3000, df=df, direction="long", market_condition="normal"
    )
    print(f"スマート出口戦略:")
    ...
    # 退出条件評価のテスト
    conditions = smart_exit.evaluate_exit_conditions(
        current_price=3050, exit_strategy=exit_strategy, direction="long"
    )
