...
class KellyCriterionSizer:
    ...
    def calculate_kelly_fraction(
        self, win_rate: float, avg_win_rate: float, avg_loss_rate: float
    ) -> float:
        """
        ケリー基準の資金比率を計算
    ...
    def calculate_position_size(
        ...
        kelly_fraction = self.calculate_kelly_fraction(
            win_rate, avg_win_rate, avg_loss_rate
        )
...
class RiskBasedSizer:
    ...
    def calculate_position_size(
        ...
        actual_risk_percentage = (
            (risk_per_share * shares) / capital if capital > 0 else 0
        )
...
class VolatilityAdjustedSizer:
    ...
    def __init__(
        self, base_risk_percentage: float = 0.02, volatility_target: float = 0.20
    ):
        """
        Args:
    ...
    def calculate_position_size(
        ...
        # 最大リスク率を制限
        adjusted_risk_percentage = min(
            adjusted_risk_percentage, self.base_risk_percentage * 2
        )
...
class CorrelationAdjustedSizer:
    ...
    def calculate_position_size(
        self, capital: float, price: float, correlation_with_portfolio: float, **kwargs
    ) -> PositionSizeResult:
        """
        相関調整でポジションサイズを計算
        ...
        correlation_factor = 1.0 - max(
            0.0, correlation_with_portfolio
        )  # 相関が1.0のとき0、0のとき1
...
class AdaptivePositionSizer:
    ...
    def calculate_optimal_position_size(
        ...
        results = [
            r
            for r in [kelly_result, risk_result, volatility_result, correlation_result]
            if r is not None
        ]
        ...
        # 最も株数が少ない保守的な方を選択
        final_result = min(
            results, key=lambda x: x.shares if x is not None else float("inf")
        )
    ...
    def record_performance(self, symbol: str, result: Dict):
        """パフォーマンスを記録"""
        record = {"symbol": symbol, "timestamp": pd.Timestamp.now(), "result": result}
...
if __name__ == "__main__":
    ...
    print(
        f"ケリー基準ポジション: {kelly_result.shares}株, 価値: {kelly_result.position_value:,.0f}円"
    )
    ...
    print(
        f"リスクベースポジション: {risk_result.shares}株, 価値: {risk_result.position_value:,.0f}円"
    )
    ...
    print(
        f"ボラティリティ調整ポジション: {vol_result.shares}株, 価値: {vol_result.position_value:,.0f}円"
    )
    ...
    print(
        f"適応型ポジション: {adaptive_result.shares}株, 価値: {adaptive_result.position_value:,.0f}円"
    )
    print(
        f"リスク率: {adaptive_result.risk_percentage:.4f}, 信頼度調整: {adaptive_result.confidence_adjustment:.2f}"
    )
