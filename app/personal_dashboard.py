...
async def add_to_portfolio(request: Request):
    ...
    return {"status": "success", "message": f"{data['symbol']} をポートフォリオに追加しました"}
...
async def add_to_watchlist(request: Request):
    ...
    """, (
        data['symbol'],
        datetime.now(),
        data.get('alert_threshold', 5.0)
    ))

    conn.commit()
    conn.close()

    return {"status": "success", "message": f"{data['symbol']} を監視銘柄に追加しました"}
...
async def optimize_portfolio():
    ...
    try:
        ...
        for position in portfolio['positions']:
            ...
            optimization_score = ((predicted_price - current_price) / current_price) * confidence
            
            optimization_results.append({
                "symbol": symbol,
                "current_shares": position['shares'],
                "optimization_score": optimization_score,
                "recommendation": "HOLD" if abs(optimization_score) < 0.02 else 
                               "BUY" if optimization_score > 0.05 else "SELL",
    ...
    try:
        ...
        comparison = {
            ...
            "symbol1": {
                ...
                "expected_return": ((pred1.get('prediction', price1) - price1) / price1 * 100) if price1 > 0 else 0
            },
            "symbol2": {
                "symbol": symbol2,
                ...
                "expected_return": ((pred2.get('prediction', price2) - price2) / price2 * 100) if price2 > 0 else 0
            },
            "recommendation": None,
            "system_accuracy": 89.18,
            "timestamp": datetime.now().isoformat()
        ...
        # 推奨決定
        if comparison["symbol1"]["expected_return"] > comparison["symbol2"]["expected_return"]:
            comparison["recommendation"] = f"{symbol1}が有利（+{comparison['symbol1']['expected_return']:.1f}%）"
        else:
            comparison["recommendation"] = f"{symbol2}が有利（+{comparison['symbol2']['expected_return']:.1f}%）"
...
async def risk_analysis(symbol: str):
    ...
    try:
        ...
        sharpe_ratio = (returns.mean() * 252) / (returns.std() * np.sqrt(252))  # シャープレシオ
        ...
        return {
            ...
            "recommendation": {
                ...
                "position_size": "大" if volatility < 0.2 else "中" if volatility < 0.3 else "小"
            },
            "timestamp": datetime.now().isoformat()
        }
