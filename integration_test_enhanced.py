...
def run_comprehensive_comparison() -> Dict[str, Any]:
    ...
    try:
        ...
        # 機能比較
        feature_results = compare_system_features(
            precision_system, enhanced_system
        )
        ...
        return {
            'success': False,
            'error': f'統合比較実行エラー: {str(e)}'
        }

def compare_prediction_systems(precision_system, enhanced_system, test_symbols: List[str]) -> Dict[str, Any]:
    """予測システムの比較"""
    print("3. 予測精度・品質比較")
    ...
    for symbol in test_symbols:
        ...
        if precision_system:
            try:
                ...
                print(f"     87%システム: 予測値={precision_data['prediction']:.1f}, "
        ...
        if enhanced_system:
            try:
                ...
                print(f"     拡張システム: 予測値={enhanced_data['prediction']:.1f}, "
...
def calculate_comparison_metrics(precision_results: List[Dict], enhanced_results: List[Dict]) -> Dict[str, Any]:
    """比較メトリクスの計算"""
    metrics = {}
    ...
    metrics['avg_prediction_time'] = {
        ...
        'improvement_ratio': np.mean(precision_times) / np.mean(enhanced_times) if enhanced_times else 1.0
    }
    ...
    for i in range(min(len(precision_results), len(enhanced_results))):
        if precision_results[i]['symbol'] == enhanced_results[i]['symbol']:
            pred_diff = abs(precision_results[i]['prediction'] - enhanced_results[i]['prediction'])
            prediction_correlations.append(pred_diff)

    metrics['prediction_consistency'] = {
        'avg_difference': np.mean(prediction_correlations) if prediction_correlations else 0.0,
        'max_difference': np.max(prediction_correlations) if prediction_correlations else 0.0
    }

    return metrics

def compare_system_performance(precision_system, enhanced_system, test_symbols: List[str]) -> Dict[str, Any]:
    """システムパフォーマンスの比較"""
    ...
    if enhanced_system:
        try:
            ...
            print(f"     拡張システム バッチ処理: {enhanced_batch_time:.3f}秒 "
    ...
    try:
        ...
        if enhanced_system:
            cache_info = {
                ...
                'parallel_workers': getattr(enhanced_system.parallel_calculator, 'n_jobs', 1)
            }
...
def display_comprehensive_results(results: Dict[str, Any]):
    ...
    if 'prediction_comparison' in results:
        ...
        if 'comparison_metrics' in prediction_comp:
            ...
            if 'avg_prediction_time' in metrics:
                ...
                print(f"     速度改善: {time_metrics.get('improvement_ratio', 1.0):.1f}倍")
    ...
    if 'performance_comparison' in results:
        ...
        if 'batch_processing' in perf_comp and 'enhanced' in perf_comp['batch_processing']:
            batch_data = perf_comp['batch_processing']['enhanced']
            print(f"   バッチ処理（拡張システム）:")
            print(f"     スループット: {batch_data.get('throughput', 0):.1f} 銘柄/秒")
            print(f"     成功率: {batch_data.get('success_count', 0)}/{len(results.get('test_symbols', []))}")
