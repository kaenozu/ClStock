...
class ModelInterpretability:
    ...
    def calculate_shap_values(self, X_sample):
        ...
        if self.explainer is None:
            raise ValueError("explainerがセットアップされていません。setup_explainer()を呼び出してください。")
    ...
    def plot_waterfall(self, shap_values, X_sample, sample_index=0, show=True):
        ...
        """
        # 特定のサンプルのSHAP値を選択
        sample_shap_values = shap_values[sample_index] if len(shap_values.shape) > 1 else shap_values
...
class AdvancedInterpretability:
    ...
    def compare_methods(self, X_sample, methods=['tree', 'linear', 'kernel']):
        ...
        for method, shap_values in shap_values_dict.items():
            plt.figure(figsize=(10, 6))
            self.interpretability.plot_feature_importance(shap_values, X_sample, show=False)
