...
class AdvancedMAPEOptimizer:
    ...
    def create_advanced_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [10, 20, 50]:
            ...
            features[f"support_resistance_{window}"] = (close - rolling_min) / (
        ...
        features["rsi_volume_interaction"] = (
            features["rsi_14"] * features["volume_ratio"]
        )
        ...
        for window in [10, 20]:
            ...
            features[f"return_normalized_{window}"] = (
                returns - mean_return
            ) / std_return
    ...
    def clean_and_select_features(
        ...
        # 目標変数と期間を合わせる
        aligned_features, aligned_target = numeric_features.align(
            target, join="inner", axis=0
        )

        # NaN除去
        combined = pd.concat(
            [aligned_features, aligned_target.rename("target")], axis=1
        )
        ...
        return (
            pd.DataFrame(X_selected, index=X.index, columns=selected_features),
            selected_features,
        )
    ...
    def train_optimized_model(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:5]:  # 計算量考慮で5銘柄
            try:
                ...
                # クリーニングと特徴量選択
                cleaned_features, selected_features = self.clean_and_select_features(
                    features, target
                )
                ...
                if cleaned_features is not None:
                    ...
                    print(
                        f"  特徴量数: {len(selected_features)}, サンプル数: {len(cleaned_features)}"
                    )
        ...
        for name, model in models.items():
            ...
            for train_idx, val_idx in tscv.split(X_clean):
                ...
                if mask.sum() > 20:  # 十分なサンプル
                    try:
                        mape = (
                            mean_absolute_percentage_error(y_val[mask], y_pred[mask])
                            * 100
                        )
