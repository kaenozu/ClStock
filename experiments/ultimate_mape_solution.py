...
class UltimateMAPESolution:
    ...
    def create_smoothed_target(
        self, data: pd.DataFrame, days_ahead: int = 5
    ) -> pd.Series:
    ...
    def calculate_proper_mape(
        self, actual: pd.Series, predicted: np.ndarray, min_threshold: float = 0.01
    ) -> float:
    ...
    def test_ultimate_system(self, symbols: List[str]) -> Dict:
        ...
        if all_mapes:
            ...
            print(f"\n分布:")
            print(
                f"  MAPE ≤ 20%: {under_20}/{len(all_mapes)} ({under_20/len(all_mapes)*100:.1f}%)"
            )
            print(
                f"  MAPE ≤ 30%: {under_30}/{len(all_mapes)} ({under_30/len(all_mapes)*100:.1f}%)"
            )
