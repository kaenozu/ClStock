...
class ConservativeMAPEAchiever:
    ...
    def create_conservative_target(
    ...
    def filter_predictable_samples(
    ...
    def calculate_strict_mape(
        self, actual: pd.Series, predicted: pd.Series, min_threshold: float = 0.01
    ) -> float:
        """厳格なMAPE計算（大きな動きのみ評価）"""
    ...
    def test_conservative_system(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:15]:  # より多くの銘柄でテスト
            try:
                ...
                # 予測可能サンプルのフィルタリング
                X_filtered, y_filtered = self.filter_predictable_samples(
                    features, target
                )
