...
def test_rule_based_predictor():
    ...
    for rec in recommendations:
        logger.info(f"[{rec.rank}位] {rec.company_name} ({rec.symbol}) - スコア: {rec.score:.1f}")
...
def test_ml_predictor():
    ...
    try:
        ...
        try:
            features, targets_reg, targets_cls = ml_predictor.prepare_dataset(test_symbols)
            logger.info(f"データセット準備完了 - 特徴量: {features.shape}, ターゲット: {targets_reg.shape}")
