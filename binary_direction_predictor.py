...
class BinaryDirectionPredictor:
    ...
    def create_binary_target(self, data: pd.DataFrame, prediction_days: int = 5) -> pd.Series:
        """バイナリターゲット（上昇=1, 下降=0）"""
        close = data['Close']
    ...
    def filter_significant_periods(self, features: pd.DataFrame, target: pd.Series,
                                 data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        """有意な期間のみをフィルタリング"""
        close = data['Close']
        returns = close.pct_change()
    ...
    def train_binary_classifier(self, X: pd.DataFrame, y: pd.Series) -> Tuple[Dict, Dict]:
        """バイナリ分類器の訓練"""
    ...
    def test_binary_system(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:15]:
            try:
                ...
                # 有意期間のフィルタリング
                X_filtered, y_filtered = self.filter_significant_periods(features, target, data)
                ...
                if minority_ratio < 0.3:
                    print(f"  スキップ: クラス不均衡 (少数クラス: {minority_ratio:.1%})")
                    continue
                ...
                # モデル訓練
                model_scores, trained_models = self.train_binary_classifier(X_train, y_train)
    ...
    def _analyze_binary_results(self, results: List[Dict]) -> Dict:
        ...
        for i, result in enumerate(top_results, 1):
            print(f"  {i}. {result['symbol']}: {result['accuracy']:.1%} ({result['best_model']})")
