...
class AdvancedDirectionalPredictor:
    ...
    def create_directional_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 移動平均の序列（強力なトレンド指標）
        features['ma_alignment_bull'] = ((sma_5 > sma_10) & (sma_10 > sma_20) & (sma_20 > sma_50)).astype(int)
        features['ma_alignment_bear'] = ((sma_5 < sma_10) & (sma_10 < sma_20) & (sma_20 < sma_50)).astype(int)
        ...
        features['macd_bullish_cross'] = ((macd > macd_signal) & (macd.shift(1) <= macd_signal.shift(1))).astype(int)

        # 4. 出来高分析
        vol_sma_20 = volume.rolling(20).mean()
        features['high_volume'] = (volume > vol_sma_20 * 1.5).astype(int)
        features['volume_confirm_up'] = ((returns_1d > 0) & (volume > vol_sma_20 * 1.2)).astype(int)
        features['volume_confirm_down'] = ((returns_1d < 0) & (volume > vol_sma_20 * 1.2)).astype(int)

        # 5. サポート・レジスタンス
        max_20 = high.rolling(20).max()
        min_20 = low.rolling(20).min()
        features['near_resistance'] = ((close > max_20 * 0.98) & (close < max_20 * 1.02)).astype(int)
        features['near_support'] = ((close < min_20 * 1.02) & (close > min_20 * 0.98)).astype(int)
    ...
    def create_directional_target(self, data: pd.DataFrame, prediction_days: int = 5,
    ...
    def train_ensemble_classifier(self, X: pd.DataFrame, y: pd.Series) -> Tuple[Dict, Dict]:
        """アンサンブル分類器の訓練"""
    ...
    def test_directional_system(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:15]:
            try:
                ...
                target = self.create_directional_target(data, prediction_days=5, threshold=0.015)
                ...
                # モデル訓練
                model_scores, trained_models = self.train_ensemble_classifier(X_train, y_train)
