...
from data.real_time_provider import (
    ReconnectionManager, DataNormalizer, RealTimeDataQualityMonitor,
    WebSocketRealTimeProvider
)
from models_refactored.core.interfaces import (
    PredictionResult as TickData, DataProvider as OrderBookData,
    ModelConfiguration as IndexData, PerformanceMetrics as NewsData
)
...
class TestRealTimeDataQualityMonitor:
    ...
    def test_get_quality_metrics(self):
        ...
        valid_order_book = OrderBookData("AAPL", datetime.now(), [(149.9, 100)], [(150.1, 150)])
        ...
        expected_metrics = [
            'tick_quality_rate', 'order_book_quality_rate',
            'total_ticks_received', 'total_order_books_received', 'data_gaps'
        ]
...
class TestWebSocketRealTimeProvider:
    ...
    async def test_connect_success(self):
        ...
        with patch('data.real_time_provider.get_settings') as mock_settings, \
             patch('data.real_time_provider.websockets.connect') as mock_connect:
    ...
    async def test_get_market_status(self):
        ...
        with patch('data.real_time_provider.get_settings') as mock_settings:
            ...
            expected_fields = [
                "is_connected", "subscribed_symbols", "subscribed_indices",
                "quality_metrics", "last_update"
            ]
