...
class TestModelPerformanceMonitor:
    ...
    def test_evaluate_model_performance(self):
        ...
        with patch('models_new.monitoring.performance_monitor.mean_squared_error') as mock_mse, \
             patch('models_new.monitoring.performance_monitor.mean_absolute_error') as mock_mae, \
             patch('models_new.monitoring.performance_monitor.r2_score') as mock_r2, \
             patch('models_new.monitoring.performance_monitor.accuracy_score') as mock_accuracy:
    ...
    def test_generate_recommendations(self):
        ...
        assert any("モデルの正則化パラメータを調整してください" in rec for rec in recommendations)
        assert any("特徴量エンジニアリングの改善を検討してください" in rec for rec in recommendations)
