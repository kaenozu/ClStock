...
class FakeProvider:
    ...
    def add_series(self, symbol: str, data: pd.DataFrame, period: str | None = None) -> None:
        key = (symbol, period)
...
def _linear_close_series(start: float, step: float, periods: int, start_date: datetime | None = None):
...
class TestDynamicStopLoss:
    def test_calculate_dynamic_stop_loss_adjusts_factors(self, risk_manager):
        ...
        assert high_confidence["confidence_factor"] < low_confidence["confidence_factor"]
...
class TestPortfolioRiskMetrics:
    def test_calculate_portfolio_var_with_deterministic_data(self, risk_manager):
        ...
        risk_manager.positions = {
            "AAA": {"size": 100, "avg_price": 95.0, "entry_date": datetime.now() - timedelta(days=30)},
            "BBB": {"size": 50, "avg_price": 205.0, "entry_date": datetime.now() - timedelta(days=45)},
        }
    ...
    def test_calculate_portfolio_var_returns_zero_with_insufficient_data(self, risk_manager):
        provider = risk_manager.data_provider
        provider.add_series("CCC", _linear_close_series(50, 1.0, 10))

        risk_manager.positions = {
            "CCC": {"size": 10, "avg_price": 48.0, "entry_date": datetime.now() - timedelta(days=5)}
        }
    ...
    def test_monitor_correlation_changes_flags_high_correlation(self, risk_manager):
        ...
        risk_manager.positions = {
            "AAA": {"size": 200, "avg_price": 98.0, "entry_date": datetime.now() - timedelta(days=20)},
            "BBB": {"size": 150, "avg_price": 148.0, "entry_date": datetime.now() - timedelta(days=40)},
        }
        ...
        assert any(pair[0] == "AAA" and pair[1] == "BBB" for pair in result["high_correlation_pairs"])


class TestKellyAndDrawdown:
    def test_calculate_position_sizing_kelly_positive_only_when_edge_crossed(self, risk_manager):
        ...
        assert positive["optimal_size"] == pytest.approx(50_000 * positive["kelly_fraction"])
        assert boundary["kelly_fraction"] == pytest.approx(0, abs=1e-12)
        assert boundary["optimal_size"] == pytest.approx(0, abs=1e-6)

    def test_assess_maximum_drawdown_risk_levels_and_recommendations(self, risk_manager):
        provider = risk_manager.data_provider
        ...
        risk_manager.positions = {
            "AAA": {"size": 300, "avg_price": 110.0, "entry_date": datetime.now() - timedelta(days=1)},
            "BBB": {"size": 200, "avg_price": 210.0, "entry_date": datetime.now() - timedelta(days=2)},
        }
...
class TestComprehensiveRiskReport:
    def test_generate_comprehensive_risk_report_aggregates_metrics(self, risk_manager, monkeypatch):
        monkeypatch.setattr(
            risk_manager,
    ...
    def test_generate_comprehensive_risk_report_handles_zero_metrics(self, risk_manager, monkeypatch):
        monkeypatch.setattr(
            risk_manager,
            "calculate_portfolio_var",
