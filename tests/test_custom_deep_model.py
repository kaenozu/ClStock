...
class TestCustomDeepModel(unittest.TestCase):
    ...
    def test_multimodal_integration(self):
        ...
        expected_features = len(self.price_data.columns) + len(self.fundamental_data.columns) + len(self.sentiment_data.columns)
    ...
    def test_attention_mechanism(self):
        ...
        multi_head_attention = MultiHeadTemporalAttention(num_heads=4, attention_units_per_head=16)
    ...
    def test_self_supervised_learning(self):
        ...
        print(f"Self-Supervised Learning - Extracted Features Shape: {extracted_features.shape}")
        ...
        # 特徴量抽出
        temporal_extracted_features = temporal_ssl_model.extract_features(self.X_time_series)
        
        # 特徴量の形状検証
        self.assertEqual(temporal_extracted_features.shape[1], 16)
        print(f"Temporal Self-Supervised Learning - Extracted Features Shape: {temporal_extracted_features.shape}")
    ...
    def test_model_interpretability(self):
        ...
        # SHAP値の計算
        shap_values = interpreter.calculate_shap_values(self.X_features[:10])  # 最初の10サンプルのみ
