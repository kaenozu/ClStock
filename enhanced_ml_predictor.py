...
class EnhancedMLPredictor:
    ...
    def create_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [5, 10, 20, 50]:
            ...
            features[f'price_to_sma_{window}'] = data['Close'] / features[f'sma_{window}'] - 1

        # ボラティリティ
        for window in [5, 20]:
            features[f'volatility_{window}'] = features['return_1d'].rolling(window).std()

        # ボラティリティ比率
        features['volatility_ratio_5'] = features['volatility_5'] / (features['volatility_20'] + 1e-8)
        features['volatility_ratio_20'] = features['volatility_20'] / (features['volatility_20'].shift(20) + 1e-8)
        ...
        features['volume_price_trend'] = features['return_1d'] * features['volume_ratio']
        ...
        features['momentum_acceleration'] = features['return_3d'] - features['return_7d']

        # サポート・レジスタンス
        for window in [20, 50]:
            features[f'high_{window}'] = data['High'].rolling(window).max()
            features[f'low_{window}'] = data['Low'].rolling(window).min()
            features[f'price_position_{window}'] = (data['Close'] - features[f'low_{window}']) / (features[f'high_{window}'] - features[f'low_{window}'])
    ...
    def prepare_training_data(self, symbol: str, lookback_days: int = 252) -> Tuple[np.ndarray, np.ndarray]:
        """訓練データの準備"""
        ...
        feature_cols = [col for col in feature_cols if not col.startswith('return_1d') or 'lag' in col]  # 未来情報除去
    ...
    def train_ensemble_model(self, symbols: List[str]) -> Dict:
        ...
        print(f"最良モデル: {best_model_name} (MAPE: {results[best_model_name]['mape']:.2f}%)")
...
def main():
    ...
    if 'error' not in results:
        ...
        for symbol in test_symbols:
            ...
            print(f"{symbol}: 予測リターン率 {predicted_return:.3f} ({predicted_return*100:.1f}%)")
