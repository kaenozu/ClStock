...
class RobustMAPEPredictor:
    ...
    def robust_mape_calculation(self, actual: np.ndarray, predicted: np.ndarray,
    ...
    def adaptive_predict_return_rate(self, symbol: str, prediction_days: int = 1) -> float:
        """適応的リターン率予測"""
        try:
    ...
    def test_robust_prediction(self, symbols: List[str], prediction_days: int = 1) -> Dict:
        """堅牢な予測システムのテスト"""
        ...
        for symbol in symbols[:5]:
            try:
                ...
                for i in range(20, prediction_days, -1):
                    ...
                    end_price = data.iloc[end_idx]['Close'] if end_idx < 0 else data.iloc[-1]['Close']
                    actual_return = (end_price - start_price) / start_price

                    # 適応的予測
                    predicted_return = self._predict_with_historical_data(historical_data, symbol, prediction_days)
    ...
    def _predict_with_historical_data(self, data: pd.DataFrame, symbol: str, prediction_days: int) -> float:
        """過去データを使った予測"""
        try:
            data = self.data_provider.calculate_technical_indicators(data)
            returns = data['Close'].pct_change().dropna()
...
def calculate_robust_metrics(results: Dict, predictor: RobustMAPEPredictor) -> Dict:
    ...
    # 堅牢MAPE計算
    robust_mape = predictor.robust_mape_calculation(actuals, predictions, min_threshold=0.01)
    ...
    if significant_cases:
        ...
        significant_mape = np.mean(np.abs((sig_actuals - sig_predictions) / sig_actuals)) * 100
    else:
    ...
    if significant_cases:
        ...
        significant_direction_accuracy = (sig_dir_actual == sig_dir_predicted).mean() * 100
    else:
    ...
    return {
        ...
        'significant_ratio': len(significant_cases) / len(predictions) if len(predictions) > 0 else 0
    }
...
def main():
    ...
    if 'error' not in metrics_1d:
        ...
        print(f"  重要動き方向性精度: {metrics_1d['significant_direction_accuracy']:.1f}%")
    ...
    for symbol in test_symbols:
        predicted_return = predictor.adaptive_predict_return_rate(symbol, prediction_days=1)
        print(f"{symbol}: 予測リターン率 {predicted_return:.4f} ({predicted_return*100:.2f}%)")
