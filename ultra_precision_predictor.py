...
class UltraPrecisionPredictor:
    ...
    def _analyze_market_context(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        # 市場の勢い
        momentum_50 = (close.iloc[-1] - close.iloc[-50]) / close.iloc[-50] if len(close) >= 50 else 0
    ...
    def _analyze_stock_characteristics(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        return {
            ...
            'volatility_adjusted_momentum': self._calculate_vol_adjusted_momentum(close, returns)
        }
    ...
    def _calculate_vol_adjusted_momentum(self, close: pd.Series, returns: pd.Series) -> float:
        """ボラティリティ調整モメンタム"""
        if len(close) < 10:
            return 0.0
    ...
    def _multi_timeframe_analysis(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        # 短期 (3-5日)
        short_trend = (close.iloc[-1] - close.iloc[-5]) / close.iloc[-5] if len(close) >= 5 else 0

        # 中期 (10-20日)
        medium_trend = (close.iloc[-1] - close.iloc[-20]) / close.iloc[-20] if len(close) >= 20 else 0

        # 長期 (50日)
        long_trend = (close.iloc[-1] - close.iloc[-50]) / close.iloc[-50] if len(close) >= 50 else 0
    ...
    def _volume_price_action_analysis(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        # 出来高トレンド
        volume_trend = volume.rolling(10).mean().pct_change(5).iloc[-1] if len(volume) >= 15 else 0

        return {
            'volume_price_correlation': np.corrcoef(returns.dropna(), volume_ratio.dropna())[0,1] if len(returns.dropna()) > 10 else 0,
    ...
    def _integrate_all_predictions(self, base_pred: Dict, market_ctx: Dict,
                                 stock_char: Dict, timeframe: Dict,
                                 volume_price: Dict, data: pd.DataFrame) -> Dict[str, float]:
        """全予測の統合"""
        ...
        if char_predictability > 0.6:
            # 予測可能性が高い場合のみ考慮
            char_direction = 1.0 if stock_char.get('volatility_adjusted_momentum', 0) > 0 else 0.0
        ...
        if total_weight == 0:
            ...
            integrated_direction = sum(d * w for d, w in zip(direction_scores, confidence_factors)) / total_weight
            integrated_confidence = total_weight

        # 超高信頼度判定
        is_ultra_confident = (
            integrated_confidence > 0.7 and
            abs(integrated_direction - 0.5) > 0.3 and
            base_pred.get('is_strong_trend', False) and
            timeframe.get('alignment_score', 0) > 0.6
        )
    ...
    def _analyze_ultra_results(self, all_results: List[Dict], ultra_results: List[Dict]) -> Dict:
        """超高精度結果分析"""
        if not all_results:
