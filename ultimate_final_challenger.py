...
class UltimateFinalChallenger:
    ...
    def identify_perfect_conditions(self, data: pd.DataFrame) -> pd.Series:
        ...
        for i in range(10, len(data)):
            if strong_uptrend.iloc[i] or strong_downtrend.iloc[i]:
                ...
                if (recent_up >= 7 or recent_down >= 7 or
                    (recent_up >= 6 and strong_momentum) or
                    (recent_down >= 6 and strong_momentum)):
    ...
    def create_ultimate_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features['price_range_position'] = (close - low.rolling(10).min()) / (high.rolling(10).max() - low.rolling(10).min())
        ...
        features['trend_consistency'] = (
            (features['ma_bullish'] & (features['sma10_slope'] > 0) & (features['price_momentum_5d'] > 0)) |
            (features['ma_bearish'] & (features['sma10_slope'] < 0) & (features['price_momentum_5d'] < 0))
        ).astype(int)
    ...
    def create_champion_ensemble(self) -> VotingClassifier:
        ...
        models = [
            ...
            # 精密調整された追加モデル
            ('lr_tuned1', LogisticRegression(
                random_state=123, max_iter=300, C=0.1, solver='liblinear'
            )),

            ('lr_tuned2', LogisticRegression(
            ...
            try:
                ...
                if champion_conf_mask.sum() > 0:
                    champion_accuracy = accuracy_score(
                        y_test[champion_conf_mask],
                        test_predictions[champion_conf_mask]
                    )
                else:
                    champion_accuracy = 0
                ...
                if champion_accuracy > 0:
                    print(f"  チャンピオン信頼度: {champion_accuracy:.1%} ({champion_conf_mask.sum()}サンプル)")
    ...
    def _analyze_champion_results(self, all_results: List[Dict], champion_results: List[Dict]) -> Dict:
        """チャンピオン結果の分析"""
        if not all_results:
        ...
        if champion_results:
            ...
            print("\n84.6%チャンピオン銘柄:")
            for r in sorted(champion_results, key=lambda x: x['accuracy'], reverse=True):
                champ_info = f" (チャンピオン信頼度: {r['champion_accuracy']:.1%})" if r['champion_accuracy'] > 0 else ""
        ...
        # 殿堂入り結果
        hall_of_fame = sorted(all_results, key=lambda x: x['accuracy'], reverse=True)[:10]
        print(f"\n殿堂入りトップ10:")
        for i, result in enumerate(hall_of_fame, 1):
            ...
            print(f"  {i}. {result['symbol']}: {result['accuracy']:.1%}{quality_info} {mark}")
