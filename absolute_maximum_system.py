...
class AbsoluteMaximumSystem:
    ...
    def identify_ultra_precise_trends(self, data: pd.DataFrame, success_pattern: Dict) -> pd.Series:
        ...
        volatility_match = (
            (volatility >= success_pattern['avg_volatility'] * 0.5) &
            (volatility <= success_pattern['avg_volatility'] * 2.0)
        )

        volume_match = (
            (volume_ratio >= success_pattern['avg_volume_ratio'] * 0.7) &
        ...
        for i in range(10, len(data)):
            if strong_uptrend.iloc[i] or strong_downtrend.iloc[i]:
                ...
                if recent_up >= 7 or recent_down >= 7:
                    # 9984成功パターンとの類似度も確認
                    if (volatility_match.iloc[i] and volume_match.iloc[i] and
                        trend_match.iloc[i] and momentum_match.iloc[i]):
                        trend_duration.iloc[i] = 1

        return trend_duration.astype(bool)

    def create_optimized_features(self, data: pd.DataFrame, success_pattern: Dict) -> pd.DataFrame:
        ...
        # 成功パターンとの類似度スコア
        features['volatility_score'] = 1.0 - abs(volatility - success_pattern['avg_volatility']) / success_pattern['avg_volatility']
        features['volume_score'] = 1.0 - abs(volume_ratio - success_pattern['avg_volume_ratio']) / success_pattern['avg_volume_ratio']
        features['trend_score'] = abs(sma_10.pct_change(5)) / success_pattern['trend_strength']
        features['momentum_score'] = abs(close.pct_change(3)) / success_pattern['price_momentum']
    ...
    def test_absolute_maximum(self, symbols: List[str]) -> Dict:
        ...
        for symbol in test_symbols:
            try:
                ...
                # 超精密トレンド期間の特定
                ultra_precise_mask = self.identify_ultra_precise_trends(data, success_pattern)
                ...
                if ultra_conf_mask.sum() > 0:
                    ultra_conf_accuracy = accuracy_score(
                        y_test[ultra_conf_mask],
                        test_predictions[ultra_conf_mask]
                    )
                else:
                ...
                if ultra_conf_accuracy > 0:
                    print(f"  超高信頼度: {ultra_conf_accuracy:.1%} ({ultra_conf_mask.sum()}サンプル)")
    ...
    def _analyze_absolute_results(self, all_results: List[Dict], absolute_results: List[Dict]) -> Dict:
        """絶対最強結果の分析"""
        if not all_results:
        ...
        if absolute_results:
            ...
            print("\n84.6%絶対突破達成銘柄:")
            for r in sorted(absolute_results, key=lambda x: x['accuracy'], reverse=True):
                ultra_info = f" (超高信頼度: {r['ultra_conf_accuracy']:.1%})" if r['ultra_conf_accuracy'] > 0 else ""
        ...
        # トップ結果
        top_results = sorted(all_results, key=lambda x: x['accuracy'], reverse=True)[:10]
        print(f"\nトップ10結果:")
        for i, result in enumerate(top_results, 1):
            mark = "***" if result['accuracy'] > 0.846 else "○○○" if result['accuracy'] >= 0.84 else "○○" if result['accuracy'] >= 0.8 else "○"
            sim_info = f" 類似度: {result['similarity_score']:.2f}"
            print(f"  {i}. {result['symbol']}: {result['accuracy']:.1%}{sim_info} {mark}")
