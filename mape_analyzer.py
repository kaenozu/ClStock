...
class MAPEAnalyzer:
    ...
    def analyze_mape_explosion(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:5]:
            try:
                ...
                for i in range(30, 5, -5):
                    ...
                    end_price = data.iloc[-i+5]['Close'] if i > 5 else data.iloc[-1]['Close']
        ...
        if problem_cases:
            ...
            print(f"問題ケースの実績リターン範囲: {problem_df['actual'].min():.4f} ～ {problem_df['actual'].max():.4f}")
            print(f"問題ケースの絶対値平均: {problem_df['abs_actual'].mean():.4f}")

            # 小さすぎる動きが問題か？
            small_movements = problem_df[problem_df['abs_actual'] < 0.01]  # 1%以下
            print(f"1%以下の小動きでの問題ケース: {len(small_movements)}/{len(problem_cases)}")

            # 最悪ケースを表示
            worst_cases = problem_df.nlargest(5, 'mape')
            print(f"\n最悪ケース Top5:")
            for _, case in worst_cases.iterrows():
                print(f"  {case['symbol']}: 実績{case['actual']:.4f} vs 予測{case['predicted']:.4f} → MAPE {case['mape']:.1f}%")
...
class ImprovedPredictor:
    ...
    def predict_return_rate_v2(self, symbol: str, prediction_days: int = 2) -> float:
        ...
        try:
            ...
            momentum_3d = (current_price - data['Close'].iloc[-4]) / data['Close'].iloc[-4] if len(data) > 4 else 0
    ...
    def test_improved_prediction(self, symbols: List[str], prediction_days: int = 2) -> Dict:
        """改善された予測システムのテスト"""
        ...
        for symbol in symbols[:5]:
            try:
                ...
                for i in range(30, prediction_days, -prediction_days):
                    ...
                    # 改善された予測
                    predicted_return = self._predict_with_data(historical_data, prediction_days)
