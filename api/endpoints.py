...
async def get_recommendations(
    ...
    try:
        ...
        return RecommendationResponse(
            ...
            market_status="市場営業時間外" if datetime.now().hour < 9 or datetime.now().hour > 15 else "市場営業中"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"推奨銘柄の取得に失敗しました: {str(e)}")
...
async def get_single_recommendation(symbol: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"推奨情報の取得に失敗しました: {str(e)}")
...
async def get_stock_data(
    symbol: str,
    period: str = Query("1y", description="期間 (1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y, 10y, ytd, max)")
):
    try:
        data_provider = StockDataProvider()

        if symbol not in data_provider.get_all_stock_symbols():
            raise HTTPException(status_code=404, detail=f"銘柄コード {symbol} が見つかりません")
        ...
        return {
            ...
            "price_change": float(technical_data['Close'].iloc[-1] - technical_data['Close'].iloc[-2]),
            "price_change_percent": float((technical_data['Close'].iloc[-1] - technical_data['Close'].iloc[-2]) / technical_data['Close'].iloc[-2] * 100),
            "volume": int(technical_data['Volume'].iloc[-1]),
            "technical_indicators": {
                "sma_20": float(technical_data['SMA_20'].iloc[-1]) if not pd.isna(technical_data['SMA_20'].iloc[-1]) else None,
                "sma_50": float(technical_data['SMA_50'].iloc[-1]) if not pd.isna(technical_data['SMA_50'].iloc[-1]) else None,
        ...
        raise HTTPException(status_code=500, detail=f"株価データの取得に失敗しました: {str(e)}")