...
class BreakthroughMAPESystem:
    ...
    def create_intelligent_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features['rsi_extremes'] = np.where(rsi_14 > 70, 1, np.where(rsi_14 < 30, -1, 0))
        ...
        # 7. 市場状況特徴量
        features['market_stress'] = vol_20 / vol_20.rolling(60).mean()  # 相対ボラティリティ
    ...
    def create_target_variables(self, data: pd.DataFrame, prediction_days: int = 7) -> pd.Series:
        """改良されたターゲット変数作成"""
        close = data['Close']
    ...
    def calculate_robust_mape(self, actual: pd.Series, predicted: pd.Series, threshold: float = 0.005) -> float:
    ...
    def test_breakthrough_system(self, symbols: List[str]) -> Dict:
        ...
        for symbol in symbols[:10]:  # より多くの銘柄でテスト
            try:
                ...
                # モデル訓練
                model_scores, trained_models = self.train_ensemble_models(X_train, y_train)
        ...
        if all_predictions:
            ...
            # 銘柄別結果
            successful_symbols = [s for s, r in symbol_results.items() if r['mape'] < 20]
