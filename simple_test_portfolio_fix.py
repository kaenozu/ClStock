...
def simple_test():
    ...
    print(f"初期: 現金={risk_manager.current_capital:,}, ポジション数={len(risk_manager.positions)}, ポトフォリオ価値={risk_manager.calculate_portfolio_value():,}")
    
    # 購入: 3000円の株を100株購入 (30万円)
    risk_manager.update_position("7203", 100, 3000, cash_flow=-300000)
    print(f"購入後: 現金={risk_manager.current_capital:,}, ポジション数={len(risk_manager.positions)}, ポトフォリオ価値={risk_manager.calculate_portfolio_value():,}")
    
    # 売却: 50株を3200円で売却 (16万円)
    risk_manager.update_position("7203", 50, 3200, cash_flow=160000)
    print(f"売却後: 現金={risk_manager.current_capital:,}, ポジション数={len(risk_manager.positions)}, ポトフォリオ価値={risk_manager.calculate_portfolio_value():,}")
    
    # 検証
    expected_value = risk_manager.current_capital + (50 * 3200)  # 現金 + 残りのポジション価値
    actual_value = risk_manager.calculate_portfolio_value()
    
    print(f"検証: 計算値({expected_value:,}) == 実際({actual_value:,}) -> {expected_value == actual_value}")
...
def test_with_debug_info():
    ...
    for symbol, pos_data in risk_manager.positions.items():
        if isinstance(pos_data, dict):
            ...
            print(f"  {symbol}: {pos_data.get('quantity')}株 × {pos_data.get('price'):,}円 = {value:,}円")
    ...
    print(f"検証: 現金({risk_manager.current_capital:,}) + ポジション({total_pos_value:,}) = {expected_total:,}")
